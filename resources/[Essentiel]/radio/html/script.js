$(function(){
    ...
    $(".toggleStatus").on('click', function(e){
        ...
        if (status == false) {
            ...
            if ($(".channel_on").val() > 0) {
                $.post('http://radio/joinChannel', JSON.stringify({channel:$(".channel_on").val()}))
            }
        } else {
            $(".radio_on").hide();
            $(".radio_off").show();
            ...
            $.post('http://radio/leaveChannel', JSON.stringify({}))
        }
    })
    ...
    $(".volumeUp").on('click', function(e){
        ...
        vol = cvolume*0.01
        if (vol == 1) vol = 0.99; 
        $.post('http://radio/setVolume', JSON.stringify({volume:vol}))
        changeActiveVolume(cvolume);
    })
    ...
    $(".volumeDown").on('click', function(e){
        ...
        if (click.paused) {click.play();} else {click.currentTime=0}
        cvolume = cvolume - 10
        $.post('http://radio/setVolume', JSON.stringify({volume:cvolume*0.01}))
        changeActiveVolume(cvolume);
    })
    ...
    $(".stepUp").on('click', function(e){
        ...
        if (click.paused) {click.play();} else {click.currentTime=0}
        var value = parseFloat($(".channel_on").val());
        newValue = parseFloat(value + .5);
        $(".channel_on").val(newValue);
        $(".channel_on").blur();
    })
    ...
    $(".validator").on('click', function(e) {
        ...
        if (click.paused) {click.play();} else {click.currentTime=0}
        var inputVal = $(".channel_on").val();
        $(".channel_on").blur();

        if (inputVal > 0) {
            $.post('http://radio/joinChannel', JSON.stringify({channel:inputVal}))
        }
    })
    ...
    window.addEventListener('message', function(e){
        if (e.data.type == "display"){
            if (e.data.status == true) {
                ...
                $.post('http://radio/close', JSON.stringify({}));
            }
        }
    })
})