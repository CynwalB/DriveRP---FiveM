...
ESX.AddGroupCommand('devinfo', '_dev', function(source, args, user)
	ESX.ChatMessage(source, "^2[^5ULS^7]^0 Groups: ^2 " .. (ESX.Table.SizeOf(ESX.Groups) - 1))
	ESX.ChatMessage(source, "^2[^5ULS^7]^0 Commands loaded: ^2 " .. (ESX.Table.SizeOf(ESX.Commands) - 1))
end)
...
ESX.AddGroupCommand('giveammo', 'superadmin', function(source, args, user)
    ...
    if currentWeapon ~= nil then
        ...
        if newAmmo < maxAmmo then
			...
			ESX.ChatMessage(source, 'Weapon player is holding already has max ammo.')
        end
    else
        ESX.ChatMessage(source, 'Player isn\'t holding a weapon.')
    end

end, {help = 'Give ammo to weapon player is holding in hands', params = {
	{name = 'playerId', help = 'Identification du joueur'},
	{name = 'count', help = 'Nombre de munitions'}
}})
