...
AddEventHandler('esx_addons_gcphone:call', function(data)
	...
	if data.message ~= nil and data.message ~= '' then
		...
		SneakyEvent('esx_addons_gcphone:startCall', data.number, data.message, {
			x = coords.x,
			y = coords.y,
			z = coords.z
		})
...
AddEventHandler('esx_basicneeds:onDrink', function(prop_name, positionx, positiony, positionz, rotationx, rotationy, rotationz)
	if not IsAnimated then
		...
		Citizen.CreateThread(function()
			...
			local prop = CreateObject(GetHashKey(prop_name), x, y, z + 0.2, true, true, true)
			local boneIndex = GetPedBoneIndex(playerPed, 18905)
			AttachEntityToEntity(prop, playerPed, boneIndex, positionx, positiony, positionz, rotationx, rotationy, rotationz, true, true, false, true, 1, true)
