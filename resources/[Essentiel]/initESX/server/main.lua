...
AddEventHandler('esx_addons_gcphone:startCall', function(number, message, coords)
	local _src = source
	--TriggerEvent("ratelimit", _src, "esx_addons_gcphone:startCall")
	if PhoneNumbers[number] ~= nil then
...
AddEventHandler('esx_phone:send', function(number, message, _, coords)
	local _src = source
	--TriggerEvent("ratelimit", _src, "esx_phone:send")
	if PhoneNumbers[number] ~= nil then
		getPhoneNumber(source, function(phone)
			notifyAlertSMS(number, {
				message = message,
...
ESX.RegisterUsableItem('redbull', function(source)
	...
	TriggerClientEvent('esx_basicneeds:onDrink', source, 'ng_proc_sodacan_01b', 0.12, -0.10, 0.08, 240.0, -60.0, 0.0) --y = 0.008
...
ESX.AddGroupCommand('removeheal', 'admin', function(source, args, user)
	if tonumber(args[1]) then
		...
		if GetPlayerName(target) then
			TriggerClientEvent('esx_status:removeTest', target)
		else
			TriggerClientEvent('chatMessage', source, "HEAL", {255, 0, 0}, "Player not found!")
		end
	else
		TriggerClientEvent('esx_status:removeTest', source)
	end
end, {help = "Heal, Nourrit et Hydrate un joueur."})
...
AddEventHandler('esx_skin:save', function(skin)
	local _src = source
	--TriggerEvent("ratelimit", _src, "esx_skin:save")
	local xPlayer = ESX.GetPlayerFromId(source)
