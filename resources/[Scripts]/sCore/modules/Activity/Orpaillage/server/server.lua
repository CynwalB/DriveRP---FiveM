...
AddEventHandler('sTerreActivity:HarvestTerre', function(item_name)
    ...
    if item_name ~= "terre" then
        ...
        if xPlayer.canCarryItem("terre", count) then
            ...
            TriggerClientEvent('esx:showNotification', source, "Vous avez récupéré ~q~"..count.." Terre~s~.")
        else
    ...
    if Terre >= 1 then
        if total >= 0 and total <= 40 then
            if xPlayer.canCarryItem("ors", 1) then
                TriggerClientEvent('esx:showNotification', source, "Vous avez trouvé ~q~1 pépite d'or~s~ dans la terre.")
        ...
        elseif total >= 80 and total <= 99 then
            if xPlayer.canCarryItem("ors", 2) then
                TriggerClientEvent('esx:showNotification', source, "Vous avez trouvé ~q~2 pépites d'or~s~ dans la terre.")
