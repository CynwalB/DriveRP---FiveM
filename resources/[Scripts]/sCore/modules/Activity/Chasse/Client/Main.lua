    ...
    while not animalDecoupe do
        ...
        if #(GetEntityCoords(PlayerPedId())-entityCoords) < 1.5 then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                ESX.ShowNotification('Vous avez ~q~réussi~s~ à découper l\'animal')
...
AddEventHandler("sChasse:returnStop", function(chasse, message)
    ...
    if message then
        ...
        RageUI.Popup({message = "Bon retour parmis ~q~nous~s~ ! J'ai pris ton fusil inutile de le chercher."})
    else
