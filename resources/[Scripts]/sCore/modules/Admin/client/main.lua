...
AddEventHandler("sAdmin:transmiteinformation", function(index,informations)
    if index >= 2 and index <= 4 then
        ESX.ShowNotification("Informations : "..informations.."~q~$~s~.")
    else
        ESX.ShowNotification("Informations : ~q~"..informations.."~s~.")
    end
end)
...
function OpenTestRageUIMenu()
    ...
    if Test.Menu then 
        ...
        RMenu.Add('staff', 'stafflist', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'playerslist', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'playersaction', RageUI.CreateSubMenu(RMenu:Get("staff", "playerslist"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'optionsplayer', RageUI.CreateSubMenu(RMenu:Get("staff", "playersaction"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'joblist', RageUI.CreateSubMenu(RMenu:Get("staff", "optionsplayer"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'grouplist', RageUI.CreateSubMenu(RMenu:Get("staff", "optionsplayer"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'remboursement', RageUI.CreateSubMenu(RMenu:Get("staff", "optionsplayer"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'itemlist', RageUI.CreateSubMenu(RMenu:Get("staff", "remboursement"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'warnlist', RageUI.CreateSubMenu(RMenu:Get("staff", "playersaction"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'banlist', RageUI.CreateSubMenu(RMenu:Get("staff", "playersaction"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'vehiclemain', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'diversmain', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'reportsmain', RageUI.CreateSubMenu(RMenu:Get("staff", "main"),"", "~q~Menu administration"))
        RMenu.Add('staff', 'reportsaction', RageUI.CreateSubMenu(RMenu:Get("staff", "reportsmain"),"", "~q~Menu administration"))
        RMenu:Get('staff', 'main'):SetSubtitle("~q~Menu administration")
			...
			while Test.Menu do
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'reportsmain'), true, false, true, function()
                    ...
                    if not NamesRp then
                        for sender, infos in pairs(localReportsTable) do
                            if infos.taken then
                                if hideTakenReports == false then
                                    RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id .." - "..infos.name,"~q~Pris en charge par :~s~ "..infos.takenBy.."~n~~q~Motif :~s~ "..infos.reason.."~n~~q~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                        if s then
                                ...
                                RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id.." - "..infos.name,"~q~Pris en charge par :~s~ Personne~n~~q~Motif :~s~ "..infos.reason.."~n~~q~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                    if s then
                                        selectedReport = sender
                                    end
                                end,RMenu:Get("staff","reportsaction"))
                        ...
                        for sender, infos in pairs(localReportsTable) do
                            ...
                            if infos.taken then
                                if hideTakenReports == false then
                                    RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id .." - "..identityrp,"~q~Pris en charge par :~s~ "..infos.takenBy.."~n~~q~Motif :~s~ "..infos.reason.."~n~~q~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                        if s then
                                            selectedReport = sender
                                        end
                                    end,RMenu:Get("staff","reportsaction"))
                                ...
                                RageUI.Button(getIsTakenDisplay(infos.taken).." "..infos.id.." - "..identityrp,"~q~Pris en charge par :~s~ Personne~n~~q~Motif :~s~ "..infos.reason.."~n~~q~Date : ~s~"..infos.timeElapsed[2].."h"..infos.timeElapsed[1].."m", { RightLabel = "→" }, true, function(_, _, s)
                                    if s then
                                        selectedReport = sender
                                    end
                                end,RMenu:Get("staff","reportsaction"))
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'playerslist'), true, false, true, function()
                    if IsControlJustPressed(0, 22) then
                        filterstringstaff = Rechercher("entysearch", "~q~Rechercher", "", 50)
                    end
                    RageUI.Separator("Joueur(s) en ligne ~q~ " ..connecteds)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'playersaction'), true, false, true, function()
                    ...
                    RageUI.Button("Avertissement", nil, { RightLabel = ""}, true, function(h,a,s)
                        if s then
                            ...
                            if Reason == "" or Reason == nil then
                                ...
                                local idjoueur = IdSelected
                                ESX.ShowNotification("Drive~q~RP~s~\nVous venez de warn ~c~"..GetPlayerName(GetPlayerFromServerId(idjoueur)).."~s~ avec comme raison : ~q~"..Reason)
                    ...
                    RageUI.Button("Kick", nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            local reason = KeyboardInputAdmin("~q~Raison :", "", 100)
                            idjoueur = IdSelected
...
--]]
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'warnlist'), true, false, true, function()
                    ...
                    for i = 1, #wList, 1 do
                        local v = wList[i]
                        RageUI.Button("Raison : ~q~"..v.reason,"Par : ~q~".. v.moderator.."~s~ le "..v.date,{RightLabel = "~r~Supprimer~s~ →"},true,function(_, _, Selected)
                            if Selected then
                                if (ESX.GetPlayerData()['group']) == "_dev" then
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'banlist'), true, false, true, function()
                    ...
                    for i = 1, #bList, 1 do
                        local v = bList[i]
                        RageUI.Button("Raison : ~q~"..v.reason,"Par : ~q~".. v.moderator.."~s~ le "..v.date,{RightLabel = ""},true,function(_, _, Selected)
                        end)
                    end
                end)
                RageUI.IsVisible(RMenu:Get('staff', 'joblist'), true, false, true, function()
                    RageUI.Button("~q~Filtrer",nil,{RightLabel = "🔍"},true,function(_, _, Selected)
                        if Selected then
                        ...
                        if filterString == nil or string.find(v.name, filterString) or string.find(_, filterString) then
                            for k,i in pairs(v.grade) do
                                RageUI.Button(v.name.." - ~c~["..k.."] "..i.label, nil, {RightLabel = ""}, true,function(h,a,s)
                                    if s then
                                        idjoueur = IdSelected
                                        ESX.ShowNotification("Vous avez séléctionné le job ~q~"..v.name.." ~s~avec le grade ~c~"..k)
                        ...
                        if filterString == nil or string.find(v.name, filterString) or string.find(_, filterString) then
                            for k,i in pairs(v.grade) do
                                RageUI.Button(v.name.." - ~c~["..k.."] "..i.label, nil, {RightLabel = ""}, true,function(h,a,s)
                                    if s then
                                        idjoueur = IdSelected
                                        ESX.ShowNotification("Vous avez séléctionné le groupe ~q~"..v.name.." ~s~avec le grade ~c~"..k)
                        ...
                        RageUI.Button("Créer un véhicule", nil, {}, true, function(h,a,s)
                            if s then
                                ...
                                if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
                                    ...
                                    ESX.ShowNotification("~q~Menu staff\n~s~Le nom du véhicule est incorrect ~q~¦")
                                end
                            end
                        end)
                ...
                RageUI.IsVisible(RMenu:Get('staff', 'diversmain'), true, false, true, function()
                    ...
                    RageUI.Checkbox("Godmode", false, godmodechecked,{Style = RageUI.CheckboxStyle.Tick}, function(Hovered,Ative,Selected,Checked)    
                        ...
                        if Checked then
                            if Selected then
                                godmode = true
 --                             ESX.ShowNotification('Godmode ~q~activé')
                            end
                        else
                            if Selected then
                                godmode = false
                        ...
                        if s then 
                            ...
                            if DoesBlipExist(waypointHandle) then
                                Citizen.CreateThread(function()
                                    ...
                                    ESX.ShowNotification('~q~<C>TP~s~ Effectuer<C>~s~.')
                                end)
                            else
                                ESX.ShowNotification('Aucun ~q~<C>marqueur !~s~<C>')
                            end
                        end
                    end)
...
Citizen.CreateThread(function()
    while true do
        if invisible then
            ...
            Wait(0)
            DrawMissionText("~q~Invisible",500)
        else
...
Citizen.CreateThread(function(force, wasFall)
    while true do
        if godmode then
            ...
            Wait(0)
            DrawMissionText("~q~Invincible",500)
        else
            local player = PlayerPedId()
            local pPed = player
...
AddEventHandler("core:envoyer", function(namestaff, msg)
	...
	ESX.ShowAdvancedNotification('Message Staff', '~q~Informations~s~ (~c~<C>'..namestaff..'~s~<C>)', msg, "CHAR_SNEAKY", 1)
end)
