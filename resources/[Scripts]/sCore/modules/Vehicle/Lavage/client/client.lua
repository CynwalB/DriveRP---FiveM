...
local function commencercarwash()
	...
	Wait(1000)
	fincarwashprops()
	ShowAboveRadarMessage("~q~CarWash\n~s~Vous avez lavé votre ~q~"..GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(pVeh))).."~s~.")
	Joshualavage = false
		...
		for _,v in pairs(poscar) do
			if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
				...
				if Vdist2(GetEntityCoords(PlayerPedId(), false), v) < 7 then
					attente = 1
					DrawText3D(v.x, v.y, v.z+0.2, "Appuyez sur ~q~E~s~ pour ~q~laver votre véhicule~s~.", 9)
