...
local healthEngineLast = 75.0
local healthBodyLast = 300.0
local healthPetrolTankLast = 1000.0

local cfg = {
	deformationMultiplier = 1.2,					-- How much should the vehicle visually deform from a collision. Range 0.0 to 10.0 Where 0.0 is no deformation and 10.0 is 10x deformation. -1 = Don't touch. Visual damage does not sync well to other players.
	...
	damageFactorEngine = 42.0,					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
	...
	engineSafeGuard = -2.0,					-- Final failure value. Set it too high, and the vehicle won't smoke when disabled. Set too low, and the car will catch fire from a single bullet to the engine. At health 100 a typical car can take 3-4 bullets to the engine before catching fire.
...
local lastVehicle
local vehicleClass
local fCollisionDamageMult = 1.0
local fDeformationDamageMult = 1.0
local fEngineDamageMult = 3.5
local fBrakeForce = 2.0
