        ...
        RageUI.IsVisible(mainMenu, true, true, true, function()
            RageUI.Button("Consulter le réservoir du véhicule", nil, {}, true, function(h, a, s)
                if s then
                    ...
                    ActualEssence = ShowAboveRadarMessage("Réservoir du véhicule : ~q~"..math.ceil(GetVehicleFuelLevel(vehicle)).."L~s~.")
                end
            end)
            NeedFuel = (100 - math.ceil(GetVehicleFuelLevel(vehicle)))
            RageUI.Button("Faire le plein du véhicule", nil, {RightLabel = "Prix : "..FuelServices.defaultPrice*NeedFuel.."~q~$~s~"}, true, function(h, a, s)
                if s then
            ...
            RageUI.Button("Acheter "..ChooseLitre.." ~o~litre d'essence~s~", "Prix : "..FuelServices.defaultPrice*ChooseLitre.."~q~$~s~", {}, true, function(h, a, s)
                if s then
...
CreateThread(function()
    ...
    while true do
        ...
        if not onRemplissage and isNearPump and IsPedInAnyVehicle(PlayerPedId(), true) then
            waiting = 1
            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder à la ~q~station service~s~.")
...
AddEventHandler("sFuel:addFuelToVehicle", function(liters)
    ...
    ESX.ShowNotification("Réaprovisionnement en cours (~q~"..math.floor(liters+0.5).."L~s~).")
    ...
    ESX.ShowNotification("Le plein a été ~q~effectué~s~.")
