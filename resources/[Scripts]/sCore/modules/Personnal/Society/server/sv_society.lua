	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent("RageUI:Popup", source, {message="Société : ~q~"..society.label.."~s~ \nType : ~q~Retrait~s~\nMontant : ~q~$"..ESX.Math.GroupDigits(amount)})
				...
				TriggerClientEvent("RageUI:Popup", source, {message="~q~Action impossible"})
			end
		end)
	else
	...
	if isBoss then
		...
		if grade ~= tonumber(getMaximumGrade(job)) or job == "unemployed" then
			if xTarget then
				...
				TriggerClientEvent("RageUI:Popup", xTarget, {message="~q~Votre profession à évoluée"})
				TriggerClientEvent("RageUI:Popup", source, {message="~q~Profession modifiée !"})
...
AddEventHandler("pSociety:RequestSetRecruit", function(target, job)
	...
	if xPlayer.job.grade_name == 'boss' then
		...
		TriggerClientEvent("RageUI:Popup", source, {message="~q~Demande envoyée."})
	end
end)
...
AddEventHandler("pSociety:SetJob", function(job, grade)
	...
	xPlayer.setJob(job, grade)
	TriggerClientEvent("RageUI:Popup", source, {message="~q~Votre profession à évoluée"})
end)