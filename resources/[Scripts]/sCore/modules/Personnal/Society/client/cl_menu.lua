...
function OpenBossMenu(_society, texturedict, texturename, options)
	RMenu.Add('bossmenu', 'main', RageUI.CreateMenu("", "Menu Patron", nil, nil,texturedict,texturename))
	...
	RMenu.Add('bossmenu', 'manage_employees', RageUI.CreateSubMenu(RMenu:Get('bossmenu', 'main'), "", "Gestion des employés"))
	RMenu.Add('bossmenu', 'update_employee', RageUI.CreateSubMenu(RMenu:Get('bossmenu', 'main'), "", "Gestion des employés"))
	RMenu.Add('bossmenu', 'manage_salary', RageUI.CreateSubMenu(RMenu:Get('bossmenu', 'main'), "", "Gestion des salaires"))
			...
			while Society.Menu do
				RageUI.IsVisible(RMenu:Get('bossmenu', 'main'), true, false, true, function()
					...
					if societymoney ~= nil then
						RageUI.Separator("Entreprise :~q~ " ..ESX.PlayerData.job.label)
						RageUI.Separator("Argent :~q~ $" ..societymoney)
					end
					...
					if options.money then
						RageUI.Button("Retirer de l'argent", false, {}, true, function(Hovered, Active, Selected)
							if Selected then
								result = KeyboardInput("pick", '~q~Combien ?', "", 8)
								if tonumber(result) then
									...
									if result > 0 then
										...
										RageUI.Popup({message = "~q~Action impossible"})
									end
								else
						...
						RageUI.Button("Déposer de l'argent", false, {}, true, function(Hovered, Active, Selected)
							if Selected then
								result = KeyboardInput("deposit", '~q~Combien ?', "", 8)
								if tonumber(result) then
									result = ESX.Math.Round(tonumber(result))
									if result > 0 then
					...
					if options.wash then
						RageUI.Button("Blanchir de l'argent", "Convertie l'argent sale en argent propre (90%).", {}, true, function(Hovered, Active, Selected)
							if Selected then
								result = KeyboardInput("wash", '~q~Combien ?', "", 8)
								if tonumber(result) then
				...
				RageUI.IsVisible(RMenu:Get('bossmenu', 'manage_employees'), true, false, true, function()
                    RageUI.Button("Rechercher", false, {RightLabel = filterstring}, true, function(Hovered, Active, Selected)
                        if Selected then
                            filterstring = KeyboardInput("entysearch", "~q~Rechercher", "", 50)
                        end
                    end)
                    RageUI.Separator("↓↓ ~q~Liste~s~ ↓↓")
                    ...
                    for i=1, #EmployeesTab do
                        ...
                        if filterstring == nil or string.find(ply.name, filterstring) or string.find(ply.job.grade_label, filterstring) then
                            if (ply.name ~= nil) then
                                RageUI.Button(ply.name, false, {RightLabel = "~q~"..ply.job.grade_label.."~s~ →"}, true, function(Hovered, Active, Selected)
                                    if Selected then
                                        RefeshjobInfos(scty)
				...
				RageUI.IsVisible(RMenu:Get('bossmenu', 'update_employee'), true, false, true, function()
					...
					RageUI.Separator("↓↓ ~q~"..SelectedEmployee.name.."~s~ ↓↓")

					for i=1, #JobTab, 1 do
						local jb = JobTab[i]

						if SelectedEmployee.job.grade ~= jb.grade then
							RageUI.Button(jb.label, false, {RightLabel = "~q~Choisir →"}, true, function(Hovered, Active, Selected)
								if Selected then
					...
					RageUI.Button("~r~Expulser de l'entreprise", false, {RightBadge = RageUI.BadgeStyle.Alert}, true, function(Hovered, Active, Selected)
						if Selected then
							result = KeyboardInput("valid", '~q~Êtes-vous vraiment sûre ?~s~ (oui)', "", 8)
							if result == "oui" then
				...
				RageUI.IsVisible(RMenu:Get('bossmenu', 'manage_salary'), true, false, true, function()
					RageUI.Separator("↓↓ ~q~"..ESX.PlayerData.job.label.."~s~ ↓↓")

					for i=1, #JobTab, 1 do
						local jb = JobTab[i]

						RageUI.Button(jb.grade..". "..jb.label, false, {RightLabel = "~q~$"..jb.salary}, true, function(Hovered, Active, Selected)
							if Selected then
								result = tostring(KeyboardInput("pick", '~q~Combien ?', "", 4))
                                result = ESX.Math.Round(tonumber(result))
								if result >= 1 then
                ...
                if not Society.Menu then
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == v.name and ESX.PlayerData.job.grade_name == "boss" then
						if mPos <= 10.0 then
							...
							if mPos <= 1.0 then
								ShowHelpNotification("Appuyez sur ~INPUT_CREATOR_RS~ pour intéragir avec ~q~l'entreprise")
	...
	Citizen.CreateThread(function()
		while Alert.Inprogress do
			...
			if IsControlPressed(0, 29) then
				RageUI.Popup({message="~q~Vous avez accepté l'offre"})
				ESX.PlayerData = ESX.GetPlayerData()
