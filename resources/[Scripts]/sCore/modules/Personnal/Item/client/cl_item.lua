...
AddEventHandler('esx_armour:armor', function()
	...
	if GetPedArmour(plyPed) == 100 then
		...
		ESX.ShowNotification("Tu as utilisé un ~q~gilet~s~ par balle.")
	end
end)
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				if CurrentAction ~= nil then
					...
					ESX.ShowNotification("Vous avez fini de ~q~réparer")
					CurrentAction = nil
...
RegisterNetEvent('esx_repairkit_moteur:onUse')
AddEventHandler('esx_repairkit_moteur:onUse', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			if IgnoreAbort then
				SneakyEvent('esx_repairkit_moteur:removeKit')
			end
			TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
...
RegisterNetEvent('esx_repairkit_caross:onUse')
AddEventHandler('esx_repairkit_caross:onUse', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed, false)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			if IgnoreAbort then
				SneakyEvent('esx_repairkit_caross:removeKit')
			end
			TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
...
AddEventHandler("Sneaky:Uselockpick",function()
	...
	if DoesEntityExist(vehicle) then
		...
		Citizen.CreateThread(function()
			...
			ESX.ShowNotification("Véhicule ~q~déverouillé~s~.")
        ...
        while true do
            ...
            for k,v in pairs(vehicle) do 
                ...
                if dst <= 1.8 and plaque == "BMX" then 
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~ramasser le bmx.")
...
function AttachCaseToPed(item_name)
    ...
    if Props.advanced then
        ...
        DrawTopNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~utiliser votre "..Props.name.."~w~.")
        Citizen.CreateThread(function()
...
function RepairVehicle(type, Entity)
    ...
    ShowAboveRadarMessage("~q~Vous avez terminé de réparer le véhicule.")
    DontSpam = false
...
AddEventHandler("SneakyLife:UseItemTires", function()
    ...
    ShowAboveRadarMessage("Vous avez réparé une ~q~roue du véhicule~s~.")
...
AddEventHandler("SneakyLife:UseItemWash", function()
    ...
    ShowAboveRadarMessage("Vous avez terminé de ~q~laver le véhicule~s~.")
