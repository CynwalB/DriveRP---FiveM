...
local ConfigLocation = {
    Positions  = {
        ...
        {
            name = "Location véhicules n°1 (~r~Vous devez posséder le permis de conduire~s~)",
        ...
        {
            name = "Location véhicules n°2 (~r~Vous devez posséder le permis de conduire~s~)",
        ...
        {
            name = "Location véhicules n°3 (~r~Vous devez posséder le permis de conduire~s~)",
        ...
        },
        {
            ...
            headingVeh = 256.51,
            Vehicles = {
                {label = "Fixter", model = "fixter", price =  50},
--                {label = "BMX", model = "bmx", price =  30},
        ...
        {
            name = "Bike Rental",
            pos = vector3(-1222.59995117188,-1495.30273438,4.3830344676971),
            zone = 3,
            banner = "bikerental",
            spawnVeh = vector3(-1225.08310546875,-1493.66127929688,4.3841414451599),
            headingVeh = 123.56,
            Vehicles = {
                {label = "BMX", model = "bmx", price = 150},
            },
        },
        ...
        {
            name = "Location bateaux n°1 (~r~Vous devez posséder le permis bateau~s~)",
        ...
        {
            name = "Location bateaux n°2 (~r~Vous devez posséder le permis bateau~s~)",
        ...
        {
            name = "Location bateaux n°3 (~r~Vous devez posséder le permis bateau~s~)",
        ...
        {
            name = "Location bateaux n°4 (~r~Vous devez posséder le permis bateau~s~)",
        ...
        {
            name = "Location bateaux n°5 (~r~Vous devez posséder le permis bateau~s~)",
        ...
        {
            name = "Location bateaux n°6 (~r~Vous devez posséder le permis bateau~s~)",
...
function spawnVehicle(table, label, name, price)
    ...
    SneakyEvent('esx_vehiclelock:givekey', 'no', plate)
--    AeroEvent("esx_vehiclelock:givekey","no",plate)
    ...
    RageUI.Popup({message = "Vous avez bien reçu votre ~q~"..label.."~s~ pour "..price.."~q~$~s~ !"})
end
...
function openLocation(table)
    if openedLocation then
        ...
        if table.zone == 1 then
            ...
            RMenu:Get('location', 'main'):SetSubtitle("Location : ~q~Voitures~s~")
        elseif table.zone == 3 then
            RMenu:Get('location', 'main'):SetSpriteBanner("root_cause","bikerental")
            RMenu:Get('location', 'main'):SetSubtitle("Location : ~q~vélos~s~")
        else
            RMenu:Get('location', 'main'):SetSpriteBanner("root_cause","shopui_title_docktease")
            RMenu:Get('location', 'main'):SetSubtitle("Location : ~q~Bateaux~s~")
        end
        ...
        Citizen.CreateThread(function()
            while openedLocation do
                RageUI.IsVisible(RMenu:Get('location', 'main'), true, false, true, function()
                    if table.zone == 1 then
                        RageUI.Separator("~q~↓~s~ Voitures à louer ~q~↓~s~")
                        for k,v in pairs(table.Vehicles) do
                            RageUI.Button(v.label, table.name, {RightLabel = "Louer → "..v.price.."~q~$"}, true, function(h,a,s)
                                if s then
                    ...
                    elseif table.zone == 3 then
                        RageUI.Separator("~q~↓~s~ Vélos à louer ~q~↓~s~")
                        for k,v in pairs(table.Vehicles) do
                            RageUI.Button(v.label, table.name, {RightLabel = "Louer → "..v.price.."~q~$"}, true, function(h,a,s)
                                if s then
                        ...
                        RageUI.Separator("~q~↓~s~ Bateaux à louer ~q~↓~s~")
                        for k,v in pairs(table.Vehicles) do
                            RageUI.Button(v.label, table.name, {RightLabel = "Louer → "..v.price.."~q~$"}, true, function(h,a,s)
                                if s then
                                    if not ESX.Game.IsSpawnPointClear(table.spawnVeh, 2.5) then
            ...
            if not openedLocation then
                for k,v in pairs(ConfigLocation.Positions) do
                    if #(myCoords-v.pos) < 1.0 then
                        ...
                        elseif v.zone == 3 then
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir la location de vélos")
                        else
