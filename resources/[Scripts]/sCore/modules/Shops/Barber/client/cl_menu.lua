...
function OpenBarberMenu()
    ...
    if Barber.Menu then 
        ...
        RMenu.Add('barber', 'characteroptionshead', RageUI.CreateSubMenu(RMenu:Get("barber", "main"),"", "Herr Kutz Barber"))
        RMenu.Add('barber', 'characteroptions_s', RageUI.CreateSubMenu(RMenu:Get("barber", "character"),"", "Herr Kutz Barber")) -- validé
        RMenu.Add('barber', 'characteroptions_h', RageUI.CreateSubMenu(RMenu:Get("barber", "characteroptionshead"),"", "Herr Kutz Barber")) -- validé
        RMenu:Get('barber', 'main'):SetSubtitle("Herr Kutz Barber")
        ...
        Citizen.CreateThread(function()
			while Barber.Menu do
                RageUI.IsVisible(RMenu:Get('barber', 'main'), true, false, true, function()
                    FreezeEntityPosition(GetPlayerPed(-1), true)
                    RageUI.Button("Barbier", "~q~Herr Kutz Barber", {RightLabel = "→"}, true,function(h,a,s)
                        if s then
                            GetComponents()
                        end
                    end,RMenu:Get("barber","characteroptionshead"))
                    RageUI.Button("Valider le changement", "~q~Herr Kutz Barber", {RightLabel = "→ 100~q~$"}, true,function(h,a,s)
                        if s then
                    ...
                    for k,v in pairs(Components) do
                        for _, s_head in pairs(sBarber) do
                            ...
                            if v.name == s_head then
                                RageUI.Button(v.label, "~q~Herr Kutz Barber", {RightLabel = "→"}, true,function(h,a,s)
                                    if a then
                                        sData = v.name
                    ...
                    for _,v in pairs(Components) do
                        if v.name == sData then
                            for i = 0, ComponentsMax[sData] do
                                RageUI.Button(v.label.." N°"..i, "~q~Herr Kutz Barber", {RightLabel = "→"}, true,function(h,a,s)
                                    if a then
                                        if sLoaded ~= i then
                                            sLoaded = i
