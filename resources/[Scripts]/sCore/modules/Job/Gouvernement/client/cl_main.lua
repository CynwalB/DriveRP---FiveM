    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, false, false, function()
            ...
            for k,v in pairs(weapons) do
                RageUI.Button("Prendre un(e) "..v.label, nil, {RightLabel = "~q~Prendre →"}, true, function(h, a, s)
                    if s then
    ...
    while true do
		...
		if ESX.PlayerData.job.name == Gouvernement.job then
			...
			elseif #(myCoords-Gouvernement.positions["garage"].deleteVehicle) < 3.5 then
				nofps = true
				ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ranger ~q~le véhicule de service~s~.")
			...
			if Gouvernement.positions["armory"].acces[ESX.PlayerData.job.grade_name] ~= nil then
				...
				if #(myCoords-Gouvernement.positions["armory"].armoryPos) < 1.5 then
					nofps = true
					ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir ~q~l'armurerie~s~.")
					if IsControlJustReleased(0, 38) then
                        if onService then
						    openArmory()
                        else
...
--[[            elseif Gouvernement.positions["avocat"].acces[ESX.PlayerData.job.grade_name] ~= nil then
                ...
                for k,v in pairs(Gouvernement.positions["avocat"].Positions) do
                    if #(myCoords-v.pos) < 1.5 then
                        nofps = true
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ouvrir ~q~le menu gestion avocat~s~.")
                        if IsControlJustReleased(0, 38) then
                            if onService then
                                openAvocat()
                            else
            ...
            for k,v in pairs(Gouvernement.positions["service"].Positions) do
                if #(myCoords-v.pos) < 1.5 then
                    nofps = true
                    if onService then label = "~r~arrêter~s~" else label = "~q~démarrer~s~" end
