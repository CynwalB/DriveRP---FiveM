...
AddEventHandler('announce:yellowjack', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('YellowJack', '~q~Informations', "- YellowJack ~g~ouvert~s~~n~- Horaire : ~q~Maintenant", "CHAR_YELLOWJACK", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('YellowJack', '~q~Informations', "- YellowJack ~r~fermé~s~~n~- Horaire : ~q~Maintenant", "CHAR_YELLOWJACK", 1)
    end
end)
...
RMenu.Add('yellowjackmenu', 'main', RageUI.CreateMenu("", "~q~Yellowjack", nil, nil,"root_cause","yellowjack"))
...
function OpenYellowjackActionMenuRageUIMenu()
    local mainMenu = RageUI.CreateMenu("", "~q~YellowJack", 0, 0,"root_cause","yellowjack")
...
RMenu:Get('garageyellowjack', 'main'):SetSubtitle("~q~Yellowjack~s~")
