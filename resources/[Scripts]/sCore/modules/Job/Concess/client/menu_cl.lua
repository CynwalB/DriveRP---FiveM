     ...
     [1] = {
          ...
          Vehicles = {
               ...
               [4] = {
                    ...
                    Desc = "Véhicules ~q~Sports Classic~s~",
                    List = {
     ...
     while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, true, false, function()
          if onService then
               ...
               RageUI.Button("Prendre son ~q~service~s~", nil, {RightLabel = "→"}, not onService, function(a,h,s)
                    if s then
                         onService = true
                    end
               end)
...
local function openCarDealerMenuCatalogue(table)
     if openedMenu then
          ...
          openedMenu = true
          if ESX.PlayerData.job.name ~= table.Job then
               RMenu:Get("cardealer", "main-catalogue"):SetSubtitle(table.Label.." - ~q~Catalogue~s~")
                    ...
                    RageUI.IsVisible(RMenu:Get("cardealer", "main-catalogue"), true, false, false, function()
                         if not cooldownCarDealer then
                              for i=1, #table.Vehicles, 1 do
                                   RageUI.Button("Catégorie : ~q~"..table.Vehicles[i].Label, nil, {RightLabel = "→→"}, not cooldownCarDealer, function(h, a, s)
                                        if s then
                         ...
                         if listVehicles ~= nil then
                              if #listVehicles == 0 then
                                   ...
                                   for k,v in pairs(listVehicles) do
                                        PU = v.price
                                        PV = v.price*1.25
                                        RageUI.Button(GetLabelText(GetDisplayNameFromVehicleModel(v.name)).." Prix d'usine: "..ESX.Math.GroupDigits(PU).."~q~$~s~", "Prix de vente: "..ESX.Math.GroupDigits(PV).."~q~$~s~", {RightLabel = "~q~Attribuer~s~ →"}, true, function(h, a, s)
                                             if s then
                                                  ...
                                                  if not ESX.Game.IsSpawnPointClear(table.Positions.spawnVeh, 3.0) then
                                                       ...
                                                       ESX.Game.SpawnVehicle(v.name, table.Positions.spawnVeh, table.Positions.spawnHeading, function(vehicle)
                                                            ...
                                                            RageUI.Popup({message = "Véhicule "..GetLabelText(GetDisplayNameFromVehicleModel(v.name)).." [~q~"..newPlate.."~s~] atrribué à ~q~"..GetPlayerName(closestPly).."~s~ !"})
                                                            openedMenu = false
                                                            RageUI.CloseAll()
                                                            FreezeEntityPosition(PlayerPedId(), false)
                    ...
                    for k,v in pairs(table.Vehicles) do
                         RageUI.IsVisible(RMenu:Get("cardealer", v.Name), true, false, false, function()
                              for k,v in pairs(v.List) do
                                   ...
                                   PV = v.price*1.25
                                   if ESX.PlayerData.job.name == table.Job then
                                        RageUI.List(GetLabelText(GetDisplayNameFromVehicleModel(v.name)), buttonList.Array, buttonList.Index, "Prix d'usine : "..PU.."~q~$~s~\nPrix vendeur : "..PV.."~q~$~s~", {}, true, function(h, a, s, i)
                                             buttonList.Index = i
                                             if i == 1 then
                                                  if s then
                                        ...
                                        RageUI.Button(GetLabelText(GetDisplayNameFromVehicleModel(v.name)), "Prix : "..PV.."~q~$~s~", {RightLabel = "→ ~q~Voir"}, true,function(h,a,s)
                                             if s then
                                                  if localVeh ~= GetHashKey(v.name) then
                                                       TriggerEvent('persistent-vehicles/forget-vehicle', Entity)
                                                       DeleteEntity(Entity)
...
local function openCarDealerMenuKey(table)
     if openedMenu then
          ...
          FreezeEntityPosition(PlayerPedId(), true)
          openedMenu = true
          RMenu:Get("cardealer", "main-key"):SetSubtitle(table.Label..' - ~q~Clés~s~')
                    ...
                    RageUI.IsVisible(RMenu:Get("cardealer", "main-key"), true, false, false, function()
                         if #getkeys == 0 then
                              ...
                              for k,v in pairs(getkeys) do
                                   ...
                                   RageUI.Button(donatedLabel..v.label, nil, {RightLabel = "~q~Créer~s~ →"}, true, function(h, a, s)
                                        if s then
...
function OpenConcessActionMenuRageUIMenu()
    ...
    if ConcessActionMenu.Menu then 
	   ...
	   RMenu.Add('concessactionmenu', 'action', RageUI.CreateSubMenu(RMenu:Get("concessactionmenu", "main"),"", "~q~Legendary Motorsport"))
        RMenu.Add('concessactionmenu', 'facture', RageUI.CreateSubMenu(RMenu:Get("concessactionmenu", "action"),"", "~q~Legendary Motorsport"))
        RMenu:Get('concessactionmenu', 'main'):SetSubtitle("~q~Legendary Motorsport")
