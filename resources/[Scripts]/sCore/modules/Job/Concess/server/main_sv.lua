    ...
    if _src then
        ...
        if Type == "open" then
            TriggerClientEvent(
                ...
                Title,
                Subtitle,
                "- "..labeljob.." ~g~Ouvert~s~~n~- Horaire : ~q~Maintenant",
                Banner
            )
        elseif Type == "close" then
            TriggerClientEvent(
                ...
                Title,
                Subtitle,
                "- "..labeljob.." ~r~Fermé~s~~n~- Horaire : ~q~Maintenant",
                Banner
            )
        elseif Type == "reduc" then
            TriggerClientEvent(
                ...
                Title,
                Subtitle,
                "- Remise disponible : ~q~10%~s~~n~- Durée : ~q~1h00",
                Banner
            )
        end
    end
...
AddEventHandler('sCardealer:addVehicule', function(Job, label, vehicle, price)
    ...
    TriggerEvent('esx_addonaccount:getSharedAccount', "society_"..Job, function(Society)
        ...
        if SocietyMoney >= price then
            ...
            TriggerClientEvent('esx:showNotification', source, "Vous avez acheter un(e) ~q~"..label.."~s~ pour ~q~"..price.."$ ~s~!")    
        else
