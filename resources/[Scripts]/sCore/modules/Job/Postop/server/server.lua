...
AddEventHandler("kPostOp:service", function(cb)
	...
	if cb then
		...
		TriggerClientEvent("esx:showNotification", player.source, "Vous venez de ~q~prendre~s~ votre ~q~service~s~ ! N'oublier pas d'allez prendre votre tenue !")
	else
		playerIsService[source] = false
		TriggerClientEvent("esx:showNotification", player.source, "Vous venez de ~q~quitter~s~ votre ~r~service~s~ !")
	end
end)
...
AddEventHandler('esx_deliveries:finishDelivery:server', function(deliveryType)
	...
	xPlayer.showNotification("Mission réussie, vous recevez ~q~"..deliveryMoney.."$~s~ !")
end)
...
AddEventHandler('esx_deliveries:removeSafeMoney:server', function(deliveryType)
	...
	if PlayerMoney >= SafeMoney then
		...
		xPlayer.showNotification("~r~Go~s~~q~Postal~s~ - Le dépôt de garantie à été retiré de votre compte bancaire")
		xPlayer.showNotification("~r~Go~s~~q~Postal~s~ - Merci de rendre le véhicule pour récupérer votre caution")
