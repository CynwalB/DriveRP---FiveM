...
AddEventHandler('announce:pizza', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("~q~Drusilla's~s~ Pizza", '~q~Informations', "- Drusilla's Pizza ~g~ouvert~s~~n~- Horaire : ~q~Maintenant", "CHAR_PIZZA", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("~q~Drusilla's~s~ Pizza", '~q~Informations', "- Drusilla's Pizza ~r~fermé~s~~n~- Horaire : ~q~Maintenant", "CHAR_PIZZA", 1)
    end
end)

function openCaisseMenuPizza()
    local mainMenu = RageUI.CreateMenu("", "~q~Caisse~s~",nil,nil,"root_cause","shopui_title_pizzathis") 
