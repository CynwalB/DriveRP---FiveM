        ...
        if ESX.PlayerData.job.name == ConfigPizza.JobName then
            if #(myCoords-ConfigPizza.Pos.Service) < 1.0 then
                ...
                if not playerInService then
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour se ~q~mettre~s~ son service")
                else
...
RMenu:Get('garagepizza', 'main'):SetSubtitle("~q~Drusilla's~s~ Pizza~s~")
...
function OpenPizzaActionMenuRageUIMenu()
    local mainMenu = RageUI.CreateMenu("", "~q~Drusilla's~s~ Pizza~s~", 0, 0,"root_cause","shopui_title_pizzathis")
