...
local function ThinkDeath(player)
    if Death.PlayerDead then
        ...
        if IsControlJustPressed(1, 47) and Death.Call then 
            ESX.ShowNotification("Vous avez appelé les ~q~secours~s~.")
        ...
        if GetGameTimer() >= Death.deathTime and Death.PlayerDead then
            ...
            Death:DrawTextScreen(0.85, 0.82, 0.8, "Appuyez sur ~q~E~s~ pour contacter les internes.", 4, 0)
            if IsControlJustPressed(1, 38) then 
                ...
                Death.PlayerDead = false 
                DoScreenFadeOut(1000)
                local dst1 = Vdist(GetEntityCoords(PlayerPedId()), 311.03410644531,-578.3524169922,43.281616210938) -- LOS SANTOS
                local dst2 = Vdist(GetEntityCoords(PlayerPedId()), 1839.1378173828,3673.2099609375,34.277328491211) -- SANDY SHORE
                if dst1 < dst2 then
                    Death:TeleportCoords(vector3(311.03410644531,-578.3524169922,43.281616210938), pPed)
                else
                    --Death:TeleportCoords(vector3(1839.1378173828,3673.2099609375,34.277328491211), pPed)
                    Death:TeleportCoords(vector3(311.03410644531,-578.3524169922,43.281616210938), pPed)
                end
    ...
    --         Death:DrawTextScreen(0.85, 0.82, 0.8, "Appuyez sur ~q~E~s~ pour vous relever.", 4, 0)
    --         if IsControlJustPressed(1, 38) then
    --             local pPed = player
    --             Death.PlayerKO = false 
    --             DoScreenFadeOut(1000)
...
AddEventHandler('SneakyLife:onPlayerDied', function(victimEntity, attackEntity)
    if victimEntity and not Death.PlayerDead and not Death.PlayerKO and IsEntityDead(victimEntity) then
        ...
        ESX.ShowNotification("Appuyez sur ~q~G~s~ pour contacter les ~q~secours~s~.")
        Death:Revive()
        SetTimecycleModifier("damage")
        SetPedToRagdoll(PlayerPedId(),1000, 1000, 0, 0, 0, 0)
...
--         ESX.ShowNotification("Vous êtes tombé ~q~KO~s~.")
...
AddEventHandler('SneakyLife:RevivePlayer', function()
    ...
    ESX.RemoveTimerBar()
    timerbaractive = false
    ShowNotification("~q~Réanimation\n~w~Vous venez d\'être réanimé.")
