...
RMenu.Add('ambulancevestiaire', 'tenues', RageUI.CreateSubMenu(RMenu:Get('ambulancevestiaire', 'main'), "", "~q~Los Santos Medical Center"))
RMenu.Add('ambulancevestiaire', 'accessories', RageUI.CreateSubMenu(RMenu:Get('ambulancevestiaire', 'main'), "", "~q~Los Santos Medical Center"))
RMenu:Get('ambulancevestiaire', 'main'):SetSubtitle("~q~Los Santos Medical Center")
				...
				RageUI.IsVisible(RMenu:Get('ambulancevestiaire', 'accessories'), true, false, true, function()
					if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") then
						for k,v in pairs(configambulance.clothsaccessories0.men) do
							RageUI.Button("Accessoire : ~q~"..v.grade, nil, {RightLabel = "→"}, true, function(h,a,s)
								if s then
									TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
										TriggerEvent('Sneakyskinchanger:loadClothes', skin, v.cloths)
									end)
        ...
        for k,v in pairs(Ambulance.PositionVestiaire) do
            ...
            if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' then
                if not AmbulanceVestiaire.Menu then
                    if mPos <= 4.0 then
                        att = 1
--                        DrawText3D(v.coords.x, v.coords.y, v.coords.z+2, "Appuyez sur ~q~E~s~ pour accéder au ~q~vestiaire~s~.", 9)
                        if mPos <= 2.0 then
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder au ~q~vestiaire~s~.")
        ...
        for k,v in pairs(Ambulance.PositionVestiairePatient) do
            ...
            if not AmbulanceVestiairePatient.Menu then
                if mPos <= 10.0 then
                    att = 1
					if mPos <= 3.5 then
                    	DrawText3D(v.coords.x, v.coords.y, v.coords.z+2, "Appuyez sur ~q~E~s~ pour prendre une ~q~blouse~s~.", 9)
					end
					if mPos <= 3.5 then
--						ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder au ~q~blouses~s~.")
						if IsControlJustPressed(0, 51) then
							ESX.PlayerData = ESX.GetPlayerData()
							pGrade = ESX.GetPlayerData().job.grade_label
            ...
            if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'ambulance' then
                if not AmbulanceGarage.Menu then
                    if mPos <= 10.0 then
                        ...
                        if mPos <= 1.5 then
							if onService then
								DrawText3D(v.coords.x, v.coords.y, v.coords.z+2, "Appuyez sur ~q~E~s~ pour accéder au ~q~garage~s~.", 9)
								ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder au ~q~garage~s~.")
								if IsControlJustPressed(0, 51) then
...
RMenu:Get('ambulancevestiairepatient', 'main'):SetSubtitle("~q~Los Santos Medical Center")
        ...
        Citizen.CreateThread(function()
            while AmbulanceVestiairePatient.Menu do
                RageUI.IsVisible(RMenu:Get('ambulancevestiairepatient', 'main'), true, false, true, function()
                    ...
                    RageUI.Separator("~q~↓~s~ Blouse ~q~↓~s~")
                    if GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") then
...
RMenu:Get('ambulancegaragehelico', 'main'):SetSubtitle("~q~Los Santos Medical Center")
...
RMenu:Get('ambulancegaragehelico2', 'main'):SetSubtitle("~q~Los Santos Medical Center")
...
RMenu:Get('ambulancegarage', 'main'):SetSubtitle("~q~Los Santos Medical Center")
...
RMenu:Get('ambulancegarage2', 'main'):SetSubtitle("~q~Los Santos Medical Center")
