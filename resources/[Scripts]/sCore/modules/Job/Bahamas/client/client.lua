...
AddEventHandler('announce:bahamas', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('Bahamas Mamas', '~q~Informations', "- Bahamas Mamas ~q~ouvert~s~~n~- Horaire : ~q~Maintenant", "CHAR_BAHAMAS", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('Bahamas Mamas', '~q~Informations', "- Bahamas Mamas ~r~fermé~s~~n~- Horaire : ~q~Maintenant", "CHAR_BAHAMAS", 1)
    end
end)
...
local MenuBahamas = {}
RMenu.Add('bahamasmenu', 'main', RageUI.CreateMenu("", "Bahamas ~q~Mamas", nil, nil,"root_cause","bahamas"))
			...
			while MenuBahamas.Menu do
                RageUI.IsVisible(RMenu:Get('bahamasmenu', 'main'), true, false, true, function()
                    if onService then
                        ...
                        RageUI.Button("Prendre son ~q~service", nil, {RightLabel = "→"}, not onService, function(h,a,s)
                            if s then
                                onService = true
                            end
                        end)
...
function OpenBahamasActionMenuRageUIMenu()
    local mainMenu = RageUI.CreateMenu("", "Bahamas ~q~Mamas", 0, 0,"root_cause","bahamas")
                ...
                if not MenuBahamas.Menu then
                    if mPos <= 10.0 then
                        ...
                        if mPos <= 2.0 then
                            ...
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour accéder aux ~q~annonces")
...
RMenu:Get('garagebahamas', 'main'):SetSubtitle("Bahamas ~q~Mamas")
    ...
    while true do
        ...
        if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'bahamas' then
            if #(pCoords-vector3(-1389.2504882812,-592.14727783203,30.319570541382)) < 2.0 then
                att = 1
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~aller~s~ au bar.")
                if IsControlJustPressed(0, 51) then
            ...
            if #(pCoords-vector3(-1385.1645507812,-606.48913574219,30.31954574585)) < 2.0 then
                att = 1
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~sortir~s~ du bar.")
                if IsControlJustPressed(0, 51) then
