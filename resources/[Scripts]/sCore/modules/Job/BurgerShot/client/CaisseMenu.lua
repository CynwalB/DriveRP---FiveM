...
AddEventHandler('announce:burgershot', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification("Burger Shot", '~q~Informations', "- Burger Shot ~q~ouvert~s~~n~- Horaire : ~q~Maintenant", "CHAR_BURGERSHOT", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification("Burger Shot", '~q~Informations', "- Burger Shot ~r~fermé~s~~n~- Horaire : ~q~Maintenant", "CHAR_BURGERSHOT", 1)
    end
end)
