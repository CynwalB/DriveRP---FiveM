...
AddEventHandler('announce:noodlee', function(announce)
    if announce == 'ouvert' then
        ESX.ShowAdvancedNotification('Noodle Exchange', '~q~Informations', "- Noodle Exchange ~r~ouvert~s~~n~- Horaire : ~q~Maintenant", "CHAR_NOODLE", 1)
    elseif announce == 'fermeture' then
        ESX.ShowAdvancedNotification('Noodle Exchange', '~q~Informations', "- Noodle Exchange ~r~fermé~s~~n~- Horaire : ~q~Maintenant", "CHAR_NOODLE", 1)
    end
end)
...
local MenuNoodle = {}
RMenu.Add('noodlemenu', 'main', RageUI.CreateMenu("", "~q~Noodle", nil, nil,"root_cause","noodle"))
...
function OpenNoodleActionMenuRageUIMenu()
    local mainMenu = RageUI.CreateMenu("", "~q~Noodle Exchange", 0, 0,"root_cause","noodle")
