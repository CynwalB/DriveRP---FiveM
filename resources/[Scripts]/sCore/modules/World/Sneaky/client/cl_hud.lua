...
--        end
    ...
    if HudState and IsPedInAnyVehicle(PlayerPedId()) then
        ...
        DrawNiceText(0.45, 0.0, 0.75, "~q~" .. GetNameFromCoords, 4)
        if pVeh and GetPedInVehicleSeat(pVeh, -1) == PlayerPedId() then
            local E, yPosition = .0600, .4
            DrawRect(E, yPosition + .003, .125, .10, 0, 0, 0, 140)
            DrawNiceText(E - .0575, yPosition - .045, 0.4, "État moteur", 6)
            DrawNiceText(E + .04, yPosition - .045, 0.4, "~q~" .. math.ceil(GetVehicleEngineHealth(pVeh) / 10) .. "%", 6, true)
            DrawNiceText(E - .0575, yPosition - .015, 0.4, "État véhicule", 6)
--            DrawNiceText(E + .04, yPosition - .015, 0.4, "~q~" .. GetVehicleHealth(pVeh) .. "%", 6, true)
            DrawNiceText(E + .04, yPosition - .015, 0.4, "~q~" .. math.ceil(GetVehicleBodyHealth(pVeh) / 10) .. "%", 6, true)
            yPosition = yPosition + .03
            DrawNiceText(E - .0575, yPosition - .015, 0.4, "Essence", 6)
            DrawNiceText(E + .0425, yPosition - .015, 0.4, "~q~" .. math.floor(GetVehicleFuelLevel(pVeh)) .. " L", 6, true)
            DrawRect(E, yPosition + .0125, .1175, .0035, 255, 123, 196, 255)
        end
    end
end