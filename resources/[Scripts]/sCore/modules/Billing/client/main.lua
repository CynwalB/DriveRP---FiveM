...
AddEventHandler("sBill:GetBill", function(bill)
    local pPed = PlayerPedId()
    ShowAboveRadarMessage("Facture: ~q~"..bill.title.."~s~.\nMontant: ~q~"..bill.price.."$~s~.")
    ShowAboveRadarMessage("Accepter: ~q~B~s~ ou Refuser: ~r~X~s~.")
    Bill.OnThread = false
...
AddEventHandler('sBill:AlertBill', function(data)
    if data == 1 then 
        ShowAboveRadarMessage("~q~La personne a payé la facture.")
    elseif data == 2 then 
