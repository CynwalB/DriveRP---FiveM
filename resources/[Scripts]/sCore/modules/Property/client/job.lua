    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, true, true, function()
            if onService then
                ...
                RageUI.List("Annonce", announce.list, announce.index, "~r~Information~s~ : faites '~q~ENTER~s~' pour passer l'annonce !", {}, true, function(h, a, s, i)
                    announce.index = i
                    if s then
                        announce.type = ""
                        if i == 1 then
    ...
    while true do
        ...
        if job then
            if #(myCoords-ConfigImmo.Pos.Interaction) < 1.5 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour gérer ~q~l'entreprise~s~.")
...
function OpenDynasty8ActionMenuRageUIMenu()
    ...
    if Dynasty8ActionMenu.Menu then 
        ...
        RMenu:Get('dynasty8', 'main'):SetSubtitle("~q~Dynasty 8")
