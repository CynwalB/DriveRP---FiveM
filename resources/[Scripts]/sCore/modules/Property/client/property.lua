...
ObjectsDetails = {
    ...
    apple = {
        pretime = 300,
        anim = {"mp_safehousewheatgrass@", "ig_2_wheatgrassdrink_michael"},
        txt = "pour ~q~boire du jus de pomme~s~",
        time = 8000
    }
}
...
local function openCreateMenu()
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, false, false, function()
            RageUI.Button("Créer une ~q~propriété~s~", nil, {RightLabel = "→"}, true, function(h, a, s)
                if s then
            ...
            if listProperties == nil then
                ...
                RageUI.Separator("↓ Liste des ~q~propriétés~s~ ↓")
                for k,v in pairs(listProperties) do
                    if v.ownerLicense ~= "none" then labelAppartenant = "~q~Oui~s~" else labelAppartenant = "~r~Non~s~" end
                    local infos = v.info
                    RageUI.Button(infos.name, "Modèle : ~o~"..infos.Selected.label.."~s~\nAppartenant à une personne : "..labelAppartenant, {RightLabel = "~q~Voir plus~s~ →"}, true, function(h, a, s)
                        if s then
                            onProperty = v
                        end
                    end, propertyInfo)
        ...
        RageUI.IsVisible(propertyInfo, true, false, false, function()
            if onProperty == nil then
                ...
                RageUI.Separator("↓ Informations de la ~q~Propriété~s~ n°~o~"..onProperty.propertyId.."~s~ ↓")
                RageUI.Separator("Type: ~o~"..infos.Selected.label)
                RageUI.Separator("Nom: ~q~"..infos.name)
                RageUI.Separator("Prix: "..infos.price.."~q~$~s~")
                if onProperty.street ~= nil then
                    RageUI.Separator("Avenue: ~q~"..onProperty.street)
                end
                if onProperty.ownerLicense == "none" then
                    RageUI.Separator("Appartenant à une personne : ~r~Non~s~")
                else
                    RageUI.Separator("Appartenant à une personne : ~q~Oui~s~")
                end
        ...
        RageUI.IsVisible(createMenuProperty, true, false, false, function()
            if not CreateProperty.Selected.state then
                ...
                for k,v in pairs(ConfigImmo.Batiment.Property) do
                    RageUI.Button(v.label, "Faites '~q~ENTRER~s~' pour sélectionner la propriété !", {}, true, function(h, a, s)
                        if s then
                ...
                RageUI.Separator("~q~Propriété~s~ - Sélectionné (~q~"..CreateProperty.Selected.label.."~s~)")
                ...
                RageUI.Button("Prix de la Propriété", nil, {RightLabel = CreateProperty.price.."~q~$~s~"}, true, function(h, a, s)
                    if s then
                ...
                RageUI.Button("Définir l'entrée", nil, { RightLabel = "~q~Définir~s~ →" }, true, function(_, _, s)
                    if s then
                        local pos = GetEntityCoords(PlayerPedId())
                        CreateProperty.pos = pos
                    end
                end)
                RageUI.Button("~q~Valider et sauvegarder", nil, {RightLabel = "→"}, true, function(h, a, s)
                    if s then
                        if CreateProperty.Selected.state == false or CreateProperty.Selected.name == "" or CreateProperty.Selected.name == nil or CreateProperty.Selected.label == "" or CreateProperty.Selected.label == nil or CreateProperty.name == "" or CreateProperty.name == nil or CreateProperty.price == 0 or CreateProperty.price == nil or CreateProperty.price == "" or CreateProperty.price < 2500 or CreateProperty.pos == "" or CreateProperty.pos == nil then 
                            ...
                            ESX.ShowNotification("~q~Création de la propriétée en cours...")
                            RageUI.GoBack()    
                        end
                    end
                end)
...
local function openManagement(owner, propertyId)
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, true, true, function()
            ...
            if owner == true then
                RageUI.Button("Gérer les clés", nil, {RightLabel = "~q~Gérer~s~ →"}, true, function(h, a, s)
                end, keysMenu)
            end

            RageUI.Button("Stockage", nil, {RightLabel = "~q~Voir et Gérer~s~ →"}, true, function(h, a, s)
                if s then
                    RageUI.CloseAll()
        ...
        RageUI.IsVisible(keysMenu, true, true, true, function()
            ...
            RageUI.Button("Personne à proximité", nil, {RightLabel = "~q~Attribuer~s~ →"}, true, function(h, a, s)
                if s then
            ...
            RageUI.Button("Destitué les clés de personnes", nil, {RightLabel = "~q~Voir~s~ →"}, true, function(h, a, s)
            end, keysMenuCivil)

            RageUI.Button("Attribuer à un des ~q~métiers~s~", nil, {RightLabel = "~q~Voir~s~ →"}, true, function(h, a, s)
            end, keysMenuJob)

            RageUI.Button("Attribuer à un des ~q~groupes~s~", nil, {RightLabel = "~q~Voir~s~ →"}, true, function(h, a, s)
            end, keysMenuGang)
        ...
        RageUI.IsVisible(keysMenuJob, true, true, true, function()
            ...
            RageUI.Separator("↓ Liste des ~q~métiers~s~ ↓")

            for k,v in pairs(jobList) do

                if returnkeysAccess[k] == nil then autorizedLabel = "~r~Non~s~" else autorizedLabel = "~q~Oui~s~" end
        ...
        RageUI.IsVisible(keysMenuGang, true, true, true, function()
            ...
            RageUI.Separator("↓ Liste des ~q~Groupes~s~ ↓")
...
--[[         RageUI.IsVisible(stockMenu, true, true, true, function()
            ...
            RageUI.Button("Déposer", nil, {RightLabel = "~q~→~s~→"}, true, function(h, a, s)
                if s then
                    ESX.PlayerData = ESX.GetPlayerData()
                end
            end, depositStock)
            
            RageUI.Button("Prendre", nil, {RightLabel = "~q~→~s~→"}, true, function(h, a, s)
                if s then
                    AeroEvent("sProperty:requestInventory", propertyId)
                end
            end, withdrawStock)
        ...
        RageUI.IsVisible(depositStock, true, true, true, function()
            ...
            if #ESX.PlayerData.inventory == 0 then            
                ...
                RageUI.Separator("↓ ~q~Votre argent~s~ ↓")

                for i = 1, #ESX.PlayerData.accounts, 1 do
                    if ESX.PlayerData.accounts[i].name == 'cash'  then
                        RageUI.Button('Argent liquide : '..ESX.PlayerData.accounts[i].money..'~q~$~s~', nil, {RightLabel = "~q~Déposer~s~ →"}, true, function(h, a, s)
                            if s then
                                local count = tonumber(PropertyInput("Veuillez saisir une ~q~quantité~s~ :", "", 6))
                    ...
                    elseif ESX.PlayerData.accounts[i].name == 'dirtycash'  then
                        RageUI.Button('Source inconnue : '..ESX.PlayerData.accounts[i].money..'~c~$~s~', nil, {RightLabel = "~q~Déposer~s~ →"}, true, function(h, a, s)
                            if s then
                                local count = tonumber(PropertyInput("Veuillez saisir une ~q~quantité~s~ :", "", 6))
                                if count == nil or count == "" or count < 1 then
                                    ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                ...
                RageUI.Separator("↓ ~q~Vos armes~s~ ↓")

                for i = 1, #ESX.PlayerData.loadout do
                    local weapon = ESX.PlayerData.loadout[i]
                    if weapon.ammo == nil then weapon.ammo = 0 end
                    RageUI.Button(weapon.label, weapon.ammo.." Munition(s)", {RightLabel = "~q~Déposer~s~ →"}, true, function(h, a, s)
                        if s then
                ...
                RageUI.Separator("↓ ~q~Votre inventaire~s~ ↓")

                for i = 1, #ESX.PlayerData.inventory do
                    local item = ESX.PlayerData.inventory[i]
                    if item.count > 0 then
                        RageUI.Button(item.label, "Quantité: ~q~"..item.count.."~s~", {RightLabel = "~q~Déposer~s~ →"}, true, function(h, a, s)
                            if s then
                                local count = tonumber(PropertyInput("Veuillez saisir une ~q~quantité~s~ :", "", 6))
                                if count == nil or count == "" or count < 1 then
                                    ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
        ...
        --[[ RageUI.IsVisible(withdrawStock, true, true, true, function()
            if stockInventory == nil then
                ...
                if #stockInventory == 0 then
                    ...
                    RageUI.Separator("↓ ~q~Stockage~s~ ↓")

                    for k,v in pairs(stockInventory) do
                        if v.type == "money" then
                            if v.name == "cash" then
                                RageUI.Button(v.label, v.count.."~q~$~s~", {RightLabel = "~q~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
                                        local count = tonumber(PropertyInput("Veuillez saisir une ~q~somme~s~ :", "", 6))
                                        if count == nil or count == "" or count < 1 then
                                            ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                                        else
                            ...
                            elseif v.name == "dirtycash" then
                                RageUI.Button(v.label, v.count.."~c~$~s~", {RightLabel = "~q~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
                                        local count = tonumber(PropertyInput("Veuillez saisir une ~q~somme~s~ :", "", 6))
                                        if count == nil or count == "" or count < 1 then
                                            ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                        ...
                        if v.type == "item" then
                            if v.count > 0 then
                                RageUI.Button(v.label, "Quantité: ~q~"..v.count.."~s~", {RightLabel = "~q~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
                                        local count = tonumber(PropertyInput("Veuillez saisir une ~q~quantité~s~ :", "", 6))
                                        if count == nil or count == "" or count < 1 then
                                            ESX.ShowNotification("~r~Erreur~s~~n~Quantité invalide !")
                        ...
                        if v.type == "weapon" then
                            if v.count > 0 then
                                RageUI.Button(v.label, "Munition(s): ~o~"..v.ammo.."~s~", {RightLabel = "~q~Prendre~s~ →"}, true, function(h, a, s)
                                    if s then
            ...
            if #(vector3(infos.pos.x, infos.pos.y, infos.pos.z)-myCoords) < 2.0 then
                ...
                if v.ownerLicense ~= "none" then
                    ...
                    if v.ownerLicense == myLicense then
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour rentrer dans ~q~la propriété~s~.")
                        ...
                        if not playerKeyAcces[v.propertyId] then
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~sonner~s~.")
                            if IsControlJustReleased(0, 54) then
                                AeroEvent("sProperty:requestInvitePlayer", v.propertyId, false)
                            end
                        else
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~entrer avec vos clés~s~.")
                            if IsControlJustReleased(0, 54) then
                    ...
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~visiter~s~.")
                    if IsControlJustReleased(0, 54) then
        ...
        while onProperty do
            ...
            for k, v in pairs(PropsInteract) do
                ...
                if object and DoesEntityExist(object) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(object), true) < 1.0 and ObjectsDetails[v] and not isInAnim then
                    nofps = true
                    DrawText3D(GetEntityCoords(object).x, GetEntityCoords(object).y, GetEntityCoords(object).z +.5, "Appuyez sur ~q~E ~s~"..ObjectsDetails[v].txt.."~s~.", 15)
            ...
            if #(pCoords-table.interiorEntry.pos) < 1.25 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~sortir de la propriété~s~.")
                if IsControlJustReleased(0, 54) then
                    onProperty = false
                end
            end
            ...
            if owner == true then
                if #(pCoords-table.managerLocation) < 1.25 then
                    nofps = true
                    ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~ouvrir la gestion de la propriété~s~.")
                    if IsControlJustReleased(0, 54) then
...
AddEventHandler("sProperty:addRequestAttribute", function(request, price)
    ...
    RageUI.PopupChar({
		message = "Paiement d'une propriété le prix : "..price.."~q~$~s~\n~q~p~s~ accepter ou ~r~q~s~ pour décliner.",
	...
	Citizen.CreateThread(function()
		...
		while InProgress do
			...
			if IsControlPressed(0, 29) then
				RageUI.Popup({message="~q~Vous avez accepté le paiement"})
				InProgress = false
				count = 0
...
AddEventHandler("sProperty:returnInvitePlayer", function(name, r)
    ...
    RageUI.PopupChar({
		message = "La personne ~q~"..name.."~s~ veut se faire inviter\n\n~q~B~s~ Accepter. | ~r~q~s~ Décliner.",
		picture = "CHAR_CHAT_CALL",
		title = "~q~Propriété~s~",
		iconTypes = 1,
		sender = "Invitation"
	...
	Citizen.CreateThread(function()
		...
		while InProgress do
			...
			if IsControlPressed(0, 29) then
				RageUI.Popup({message="~q~Vous avez accepté l'invitation"})
				InProgress = false
				count = 0
