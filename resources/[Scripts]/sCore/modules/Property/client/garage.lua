    ...
    while mainMenu do
        ...
        RageUI.IsVisible(garageInfo, true, false, false, function()
            if onGarage == nil then
                ...
                RageUI.Separator("↓ Informations du ~o~Garage~s~ n°~q~"..onGarage.garageId.."~s~ ↓")
                ...
                if onGarage.street ~= nil then
                    RageUI.Separator("Avenue: ~q~"..onGarage.street)
                end
        ...
        RageUI.IsVisible(createMenuGarage, true, false, false, function()
            if not CreateGarage.Selected.state then
                ...
                for k,v in pairs(ConfigImmo.Batiment.Garage) do
                    RageUI.Button(v.label, "Faites '~q~ENTER~s~' pour sélectionner le garage !", {}, true, function(h, a, s)
                        if s then
                ...
                RageUI.Separator("~o~Garage~s~ - Sélectionné (~q~"..CreateGarage.Selected.label.."~s~)")
                ...
                RageUI.Button("Prix du Garage", nil, {RightLabel = CreateGarage.price.."~q~$~s~"}, true, function(h, a, s)
                    if s then
...
local function openManagementVehicle(vehicles)
    ...
    while mainMenu do
        ...
        RageUI.IsVisible(mainMenu, true, false, false, function()
            if #vehicles > 0 then
                RageUI.Separator("↓ Liste de vos ~q~véhicules~s~ ↓")
                for k,v in pairs(vehicles) do
                    props = v.props
                    RageUI.Button(GetLabelText(GetDisplayNameFromVehicleModel(props.model)), "Plaque: ~q~"..props.plate, {RightLabel = "~q~Voir plus~s~ →"}, true, function(h, a, s)
                        if s then
                            infoVeh = v
                        end
                    end, infosVehicle)
        ...
        RageUI.IsVisible(infosVehicle, true, false, false, function()
            if infoVeh == nil then
                ...
                props = infoVeh.props
                RageUI.Separator("Modèle du véhicule: ~q~"..GetLabelText(GetDisplayNameFromVehicleModel(props.model)))
                RageUI.Separator("Plaque du véhicule: ~q~"..props.plate)
                RageUI.Separator("Essence du véhicule: "..math.ceil(props.fuelLevel).."~q~/~s~100%")
                RageUI.Separator("État du véhicule: "..math.ceil(props.bodyHealth).."~q~/~s~1000%")
                RageUI.Separator("État du moteur: "..math.ceil(props.engineHealth).."~q~/~s~1000%")
            end
        end)
                ...
                if v.ownerLicense ~= "none" then
                    ...
                    if v.ownerLicense == myLicense then
                        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour rentrer dans ~q~le garage~s~.")
                            if IsControlJustReleased(0, 54) then
                                AeroEvent("sGarage:enter", v.garageId, true)
                            end
                        else
                            ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour rentrer votre ~q~véhicule~s~ dans le garage.")
                            if IsControlJustReleased(0, 54) then
    ...
    CreateThread(function()
        ...
        while onGarage do
            ...
            if owner and #(pCoords-table.manager) < 1.25 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~gérer vos véhicules~s~.")
                if IsControlJustReleased(0, 54) then
            ...
            if #(pCoords-table.entry.pos) < 1.0 then
                nofps = true
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~sortir du garage~s~.")
                if IsControlJustReleased(0, 54) then
                    stopVeh = false
                    onGarage = false
                end
...
AddEventHandler("sGarage:addRequestAttribute", function(request, price)
    ...
    RageUI.PopupChar({
		message = "Paiement d'un garage le prix : "..price.."~q~$~s~\n~q~p~s~ accepter ou ~r~q~s~ pour décliner.",
...
AddEventHandler("sGarage:outVehicle", function(table, props)
    ...
    ESX.ShowNotification("~q~Information~s~~n~Vous venez de sortir avec votre véhicule ~o~"..GetLabelText(GetDisplayNameFromVehicleModel(props.model)).."~s~ ["..props.plate.."] !")
