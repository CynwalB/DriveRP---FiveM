...
function Drugs:StartBoucleForSelling()
    ...
    while true do
        ...
        while deliveryPos == nil do
            Wait(0)
            Drugs:DrawText("Vous êtes a la recherche de ~q~clients~s~.", 1)
        ...
        if deliveryPos ~= nil and delivery == nil then
            ...
            Interval = 0
            Drugs:DrawText("Un point de livraison a été marqué dans la ~q~zone~s~.", 1)
            if #(GetEntityCoords(PlayerPedId())-delivery["point"]) < 3.0 then
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~q~déposer~s~ votre ~q~livraison~s~.")
