...
AddEventHandler("sLaboratoire:createLab", function(type, name, price, pos)
    if source then
        ...
        MySQL.Async.execute('INSERT INTO lab_list (type, name, price, pos, time) VALUES (@type, @name, @price, @pos, @time)', {
            ...
            TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous venez de créer un ~q~"..name.."~s~ à "..price.."~q~$~s~ !")
            TriggerClientEvent("sLaboratoire:refreshLab", -1)
       end) 
    end
end)
        ...
        MySQL.Async.fetchAll('SELECT * FROM lab_list WHERE `type` = @type AND `id` = @id', {
            ...
            if result[1] then
                MySQL.Async.execute("DELETE FROM lab_list WHERE `type` = @type AND `id` = @id", {
                    ...
                    TriggerClientEvent("esx:showNotification", xPlayer.source, "Le laboratoire n°~q~"..id.."~s~ viens d'être ~r~supprimé~s~ !")
                    TriggerClientEvent("sLaboratoire:refreshLab", -1)
                end)
            else
                TriggerClientEvent("esx:showNotification", xPlayer.source, "~r~Le laboratoire n°~q~"..id.."~s~ n'existe pas !")
            end
       end)
    end
end)
    ...
    if source then
        ...
        if xPlayer.getAccount("cash").money >= table.price then
            MySQL.Async.execute('UPDATE lab_list SET owner = @owner, ownerName = @ownerName, time = @time WHERE id = @id', {
                ...
                TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous venez de acheter un ~q~"..table.name.."~s~ pour "..table.price.."~q~$~s~")
...
AddEventHandler("sLaboratoire:removeOwner", function(table, owner, ownerName)
    if source then
        ...
        MySQL.Async.execute('UPDATE lab_list SET owner = @owner, ownerName = @ownerName WHERE id = @id', {
            ...
            TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous venez de dissocier le ~q~"..table.name.."~s~ des ~q~"..ownerName.."~s~ !")
            TriggerClientEvent("sLaboratoire:returnBuyLab", xPlayer.source)
            TriggerClientEvent("sLaboratoire:refreshLab", -1)               
        end)
    ...
    if source then
        ...
        for i = 1, #xPlayers do
            ...
            if xPlayers.job2.name == xPlayer.job2.name then
                TriggerClientEvent("esx:showNotification", xPlayers.source, "Vous êtes en train ~r~d'attaquer~s~ le laboratoire appartenant au ~q~"..ownerName.."~s~.")
                TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous êtes en train ~r~d'attaquer~s~ le laboratoire appartenant au ~q~"..ownerName.."~s~.")
            end
            if xPlayers.job2.name == owner then
                TriggerClientEvent("esx:showNotification", xPlayers.source, "Votre laboratoire situé à ~q~"..zone.."~s~ est actuellement en train de se faire ~r~attaquer~s~ aller le défendre au plus vite.")
            end
        end
    end
end)
...
AddEventHandler("sLaboratoire:updateAttack", function(id, name, owner, ownerName)
    if source then
        ...
        for i = 1, #xPlayers do
            ...
            local xPlayers = ESX.GetPlayerFromId(xPlayers[i])
            if xPlayers.job2.name == xPlayer.job2.name then
                TriggerClientEvent("esx:showNotification", xPlayers, "Vous venez de capturer le laboratoire de ~q~"..name.."~s~.")
                TriggerClientEvent("esx:showNotification", source, "Vous venez de capturer le laboratoire de ~q~"..name.."~s~.")
            end
        end
        TriggerClientEvent("sLaboratoire:refreshLab", -1)     
    end
