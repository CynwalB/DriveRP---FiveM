...
AddEventHandler("sDrugs:addItem", function(itemName)
    ...
    if xPlayer then
        ...
        if xPlayer.canCarryItem(itemName, Allow.Items[itemName].count) then
            ...
            TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous avez récolté ~q~x"..Allow.Items[itemName].count.." "..ESX.GetItemLabel(itemName).."~s~ !")
        else
...
AddEventHandler("sDrugs:transformItem", function(lastItem, itemName)
    ...
    if xPlayer then
        ...
        xPlayer.addInventoryItem(itemName, Allow.Items[itemName].count)
        TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous avez transformé ~q~x"..Allow.Items[lastItem[1]].count.." "..ESX.GetItemLabel(lastItem[1]).."~s~ et ~q~x"..Allow.Items[lastItem[2]].count.." "..ESX.GetItemLabel(lastItem[2]).."~s~ !")
    end
end)
...
AddEventHandler("sDrugs:pochonItem", function(lastItem, itemName)
    ...
    if xPlayer then
        ...
        xPlayer.addInventoryItem(itemName, Allow.Items[itemName].count)
        TriggerClientEvent("esx:showNotification", xPlayer.source, "Vous avez mis en pochon ~q~x"..Allow.Items[lastItem].count.." "..ESX.GetItemLabel(lastItem).."~s~ !")
    end
end)
...
AddEventHandler("sDrugs:Removeitem",function()
    ...
    if xPlayer.canCarryItem(itemName, count) then
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, "Vous avez transformé ~q~x"..countRemove.." "..ESX.GetItemLabel(itemRemove).."~s~.")
    else
