        ...
        Citizen.CreateThread(function()
            while openedCreatedMenu do
                ...
                RageUI.IsVisible(createdMenu, false, false, true, function()
                    RageUI.Button("Liste des laboratoires", nil, {RightLabel = "~q~Voir~s~ →"}, true, function(_, _, Select)
                        if (Select) then
                    ...
                    RageUI.Separator("")
                    RageUI.Separator("↓ Création d'un ~q~laboratoire~s~ ↓")
                    if not createLab.selectLabo then
                        ...
                        RageUI.Separator("Laboratoire choisi ~q~"..createLab.selectLaboLabel)
                    end
                    RageUI.Button("Prix", nil, {RightLabel = "→ "..createLab.price.."~q~$"}, createLab.selectLabo, function(_, _, Select)
                        if (Select) then
                            local amount = createdLabInput("Veuillez saisir un prix :", "", 6)
                            if tonumber(amount) then
                                if tonumber(amount) >= 1500 then
                                    ESX.ShowNotification("Prix fixé sur "..amount.."~q~$~s~ !")
                                    createLab.price = tonumber(amount)
                                else
                                    ESX.ShowNotification("Veuillez entrer un nombre plus haut que 1500~q~$~s~ !")
                                end
                            else
                        ...
                        if #resultLab == 0 then
                            ...
                            for k,v in pairs(resultLab) do
                                if v.owner == nil then
                                    ...
                                    ownerName = "au ~q~"..v.ownerName.."~s~"
                                end
                                RageUI.Button(v.name.." - "..v.price.."~q~$~s~", "Appartenant "..ownerName..", laboratoire n°"..v.id, {RightLabel = "~q~Voir~s~ →"}, true, function(_, _, Select)
                                    if (Select) then
                ...
                RageUI.IsVisible(createdViewMenu, false, false, true, function()
                    RageUI.Separator("Laboratoire n°~q~"..selectLab.id)
                    RageUI.Separator("Type de laboratoire ~q~"..selectLab.type)
                    if selectLab.ownerName ~= nil then
                        RageUI.Separator("Appartenant "..selectLab.ownerName)
                    else
                        RageUI.Separator("Appartenant "..ownerName)
                    end
                    RageUI.Separator("Prix du laboratoire "..selectLab.price.."~q~$~s~")
                    RageUI.Button("Se téléporter", "~r~Attention~s~ : Pas de retour possible !", {RightLabel = "~q~Oui~s~ →"}, true, function(_, _, Select)
                        if (Select) then 
                            local pos = json.decode(selectLab.pos)
                            SetEntityCoords(PlayerPedId(), pos.x, pos.y, pos.z-1.0)
                            ESX.ShowNotification("Vous vous êtes bien ~q~téléporté~s~ au ~q~"..selectLab.name.."~s~ n°~q~"..selectLab.id.."~s~ !")
                            RageUI.CloseAll()
                            openedCreatedMenu = false
                        end
                    end)
                    if selectLab.owner ~= nil then
                        RageUI.Button("Dissocier le laboratoire des ~q~"..selectLab.ownerName, "~r~Attention~s~ : Pas de retour possible !", {RightLabel = "~q~Oui~s~ →"}, true, function(_, _, Select)
                            if (Select) then 
