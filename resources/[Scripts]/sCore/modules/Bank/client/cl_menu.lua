...
RMenu.Add('banque', 'main', RageUI.CreateMenu("", "~q~Banque", 10, 200,'root_cause',"shopui_title_fleecabank"))
        ...
        Citizen.CreateThread(function()
			while Bank.Menu do
                RageUI.IsVisible(RMenu:Get('banque', 'main'), true, false, true, function()
                    if zone == 1 then
                        RageUI.Separator("Solde du compte : "..soldaccount.."~q~$")
            ...
            if not Bank.Menu then
                if #(myCoords-v.pos) < 10.0 then
                    ...
                    if #(myCoords-v.pos) < 1.5 then
                        zone = 1
                        ESX.ShowHelpNotification("Appuyez sur ~INPUT_PICKUP~ pour accéder à la ~q~banque~s~.")
