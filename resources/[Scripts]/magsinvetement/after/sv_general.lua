...
ESX.RegisterServerCallback('Checkmoney', function(source, cb, price)
	...
	if xPlayer.getAccount('cash').money >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "~g~Vous avez effectuer un payement de ~s~(~q~$"..tonumber(price).."~s~)")  
	else 
...
AddEventHandler('SneakyLife:InventoryItem', function(id, target, item)
	...
	targetXPlayer.showNotification("Vous venez de recevoir un/une ~r~"..item)
	--TriggerClientEvent('esx:showNotification', xaplayertarget.source, "~r~Vous venez de recevoir un/une ~q~ "..item)
end) 
