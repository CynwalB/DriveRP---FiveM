...
local menu = {
	mainClothes = { 
		["T-shirt"] = {name = "T-shirt", ask = "→→", askX = true, id = 'Selection', data = 'tshirt_1', data2 = 'tshirt_2', variation = 8, variation2 = 'texture', price = 20, realName = "peelotshirt"},
		["Torse"] = {name = "Torse", ask = "→→", askX = true, id = 'Selection', data = 'torso_1', data2 = 'torso_2', variation = 11, variation2 = 'texture', price = 20, realName = "peelotorse"},
		["Pantalon"] = {name = "Pantalon", ask = "→→", askX = true, id = 'Selection', data = 'pants_1', data2 = 'pants_2', variation = 4, variation2 = 'texture', price = 20, realName = "peelopantalon"},
		["Chaussures"] = {name = "Chaussures", ask = "→→", askX = true, id = 'Selection', data = 'shoes_1', data2 = 'shoes_2', variation = 6, variation2 = 'texture', price = 15, realName = "peelochaussures"},
		["Calques"] = {name = "Calques", ask = "→→", askX = true, id = 'Selection', data = 'decals_1', data2 = 'decals_2', variation = 1, variation2 = 'props', price = 10, realName = "peeloCalques"}, 
		["Gants"] =	{name = "Gants", ask = "→→", askX = true, id = 'Selection', data = 'arms', data2 = 'arms_2', variation = 1, variation2 = 11, price = 5, realName = "peelogant"}, 
	},   
	accessories = { 
		...
		["Sac à dos"] =	{name = "Sac à dos", ask = ">", askX = true, id = 'Selection', data = 'bags_1', data2 = 'bags_2', variation = 5, variation2 = 'texture', price = 35, realName = "peelosac"},
		...
		["Gilet"] =	    {name = "Gilet", ask = ">", askX = true, id = 'Selection', data = 'bproof_1', data2 = 'bproof_2', variation = 0, variation2 = 'props', price = 35, realName = "peeloogilet"},
	},
	extra = {
		["Masque"] = {name = "Masque", id = "extra", data = "mask_1", data2 = "mask_2", realName = "peelomasque", variation = 1, variation2 = "texture", price = 40}
	}
}    
...
function openClotheMenu()
	if Clothes.Menu then 
		...
		RMenu.Add("publish", "main", RageUI.CreateSubMenu(RMenu:Get("clothes", "main"), "", "~o~Ponsonbys"))
		RMenu.Add("masque", "main", RageUI.CreateMenu("","~o~Masque",0,0,"root_cause","shopui_title_highendfashion"))
		RMenu.Add("compactation", "main", RageUI.CreateSubMenu(RMenu:Get("clothes", "main"), "", "~o~Ponsonbys"))
		RMenu.Add("mainClothes", "main", RageUI.CreateSubMenu(RMenu:Get("clothes", "main"), "", "~o~Ponsonbys"))
		RMenu.Add("accessories", "main", RageUI.CreateSubMenu(RMenu:Get("clothes", "main"), "", "~o~Ponsonbys"))
		RMenu.Add("subMenu", "main", RageUI.CreateSubMenu(RMenu:Get("clothes", "main"), "", "~o~Ponsonbys"))
									...
									ESX.TriggerServerCallback('Neo:getalltenues', function(Vetement)
										for k, v in pairs(Vetement) do  
											if v.type == "peelopublic" then 
												table.insert(shoplunettes.Menu["Publier/liste des tenue"].b, {name = v.nom, price = 150, skins = json.decode(v.clothe)})
											end
										end
									end)
								end
...
--[[							RageUI.Button("Sauvegarder la tenue actuelle", nil, { RightLabel = "~g~$200" }, true, function(s, p, q)
								if (q) then 
					...
					RageUI.IsVisible(RMenu:Get("mainClothes","main"), true, true, true,function()
						...
						RageUI.Separator("~r~Vous devez sauvegarder votre tenue pour")
						RageUI.Separator("~r~qu'elle soit dans votre inventaire (dans ce")
						RageUI.Separator("~r~cas là, pas besoin de payer chaque vêtement)")
...
--						RageUI.Button("Sauvegarde votre tenue actuelle", nil, { RightLabel = '~g~$100' }, not cooldown, function(s, p, q)
						RageUI.Button("Sauvegarde votre tenue actuelle", nil, { RightLabel = '→→' }, not cooldown, function(s, p, q)
							if (q) then 
								ESX.TriggerServerCallback('Checkmoney', function(cb)
									if cb then 
										save() 
										...
										TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
											...
											if check then 
												TriggerEvent('Sneakyskinchanger:loadClothes', skin, { 
													...
													["decals_1"] = v.skins["decals_1"],
													["decals_2"] = v.skins["decals_2"],
													["torso_2"] = v.skins["torso_2"],
													["chain_1"] = v.skins["chain_1"],
													["chain_2"] = v.skins["chain_2"],
													["bags_1"] = v.skins["bags_1"],
													["bags_2"] = v.skins["bags_2"],
													["bproof_1"] = v.skins["bproof_1"],
													["bproof_2"] = v.skins["bproof_2"],
													["ears_1"] = v.skins["ears_1"],
													["ears_2"] = v.skins["ears_2"],
									...
									if q and ESX.GetPlayerData()['group'] ~= "user" then 
										...
										if result ~= nil then  
											TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
												...
												ESX.ShowNotification('Vous avez (~b~Publier~s~) votre tenue [~g~'..result..'~s~] ')
											end)
										end
									end 
								end)
...
function savetenue()
	TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
		...
		ESX.ShowNotification('Vous avez (~b~engeristrer~s~) votre tenue [~g~'..math..'~s~] ')
	end)
end
