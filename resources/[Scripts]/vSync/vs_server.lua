    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                ...
                TriggerClientEvent('vSync:notify', source, 'Time is ~g~no longer frozen~s~.')
            end
        else
    ...
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    TriggerClientEvent('vSync:notify', source, 'Weather will change to: ~g~' .. string.lower(args[1]) .. "~s~.")
                    CurrentWeather = string.upper(args[1])
                    newWeatherTimer = 10
                    TriggerEvent('vSync:requestSync')
                else
...
RegisterCommand('morning', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(9)
        TriggerClientEvent('vSync:notify', source, 'Time set to ~g~morning~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('noon', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(12)
        TriggerClientEvent('vSync:notify', source, 'Time set to ~g~noon~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('evening', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(18)
        TriggerClientEvent('vSync:notify', source, 'Time set to ~g~evening~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('night', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(23)
        TriggerClientEvent('vSync:notify', source, 'Time set to ~g~night~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
    ...
    elseif source ~= 0 then
        if isAllowedToChange(source) then
            if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
                ...
                TriggerClientEvent('vSync:notify', source, 'Time was changed to: ~g~' .. newtime .. "~s~!")
                TriggerEvent('vSync:requestSync')
            else
