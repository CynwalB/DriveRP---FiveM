...
AddEventHandler("esx_inventoryhud:openStorageInventory", function(job)
    CurrentJob = job
    ESX.TriggerServerCallback("Sneaky:getStockItems", function(item,weapon,dirtycash,cash)
        setStorageInventoryData({inventory = item, weapons = weapon, blackMoney = dirtycash, money = cash})
...
RegisterNUICallback("PutIntoStorage",function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        ...
        if data.item.type == "Argent de source inconnue" then
