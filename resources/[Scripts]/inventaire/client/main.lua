...
RegisterNUICallback("RenameItem", function(data, cb)
    if data.item.type ~= "item_weapon" then 
        ...
        if result ~= nil then 
            ...
            ESX.ShowNotification('Vous avez changer le nom ~c~'..data.item.label..'~s~ en ~q~'..result..'~s~')
        end 
    else
        closeInventory()
        local result = KeyboardInput('', data.item.label, 30)
...
RegisterNUICallback("InformationItem", function(data, cb)
    ESX.ShowNotification('~q~Type : ~s~'..data.item.type..'\n~q~Nombre : ~s~'..data.item.count..'\n~q~Nom : ~s~'..data.item.label..'')
end)


local notifItem = false

RegisterNetEvent('inventaire:animationinde')
AddEventHandler('inventaire:animationinde', function()
    Citizen.CreateThread(function()
...
--        TaskPlayAnim(ped, 'move_m@_idles@shake_off', 'move_m@_idles@shake_off', 8.0, -8, 10.0, 49, 0, 0, 0, 0)
        ...
        TaskPlayAnim(ped, 'clothingtie', 'try_tie_positive_a', 8.0, -8, 10.0, 49, 0, 0, 0, 0)
        Citizen.Wait(3500)
        ClearPedSecondaryTask(ped)
    end)
end)

RegisterNetEvent('inventaire:animation')
AddEventHandler('inventaire:animation', function()
    Citizen.CreateThread(function()
        local ped = PlayerPedId()
--        RequestAnimDict('move_m@_idles@shake_off') paper_1_rcm_alt1-9
--        TaskPlayAnim(ped, 'move_m@_idles@shake_off', 'move_m@_idles@shake_off', 8.0, -8, 10.0, 49, 0, 0, 0, 0)
        RequestAnimDict('clothingtie')
        TaskPlayAnim(ped, 'clothingtie', 'try_tie_neutral_d', 8.0, -8, 10.0, 49, 0, 0, 0, 0)
        Citizen.Wait(3500)
        ClearPedSecondaryTask(ped)
    end)
end)
    ...
    elseif data.item.type == "item_tenue" then 
        TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
            if tenue then 
                ...
                TriggerEvent('inventaire:animation')
                Citizen.Wait(3000)
                ...
                ESX.ShowNotification("Vous venez d'équipé : ~q~"..data.item.label)
        ...
        TriggerEvent('Sneakyskinchanger:getSkin', function(skin)
            if used then
                ...
                if data.item.decals ~= nil then 
                    TriggerEvent('inventaire:animationinde')
                    Citizen.Wait(3500)
                ...
                notifItem = ESX.ShowNotification("Vous venez d'équipé : ~q~"..data.item.label)
...
RegisterNUICallback("DropItem", function(data, cb)
    ...
    if currentMenu ~= 'clothe' then 
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            if data.item.label == "Argent de source inconnue" then
...
RegisterNUICallback("GiveItem", function(data, cb)
    ...
    if foundPlayer then
        ...
        -- TaskPlayAnim(GetPlayerPed(GetClosestPlayer(2)), 'mp_common', 'givetake1_a', 8.0, -8, 10.0, 49, 0, 0, 0, 0)
        -- Citizen.Wait(1200)
        -- ClearPedSecondaryTask(GetPlayerPed(GetClosestPlayer(2)))
        ...
        if data.item.type == "item_tenue" or data.item.type == "item_chaussures" or data.item.type == "item_calque" or data.item.type == "item_montre" or data.item.type == "item_chaine" or data.item.type == "item_masque" or data.item.type == "item_pantalon" or data.item.type == "item_chapeau" or data.item.type == "item_sac" then
            TriggerServerEvent('SneakyLife:InventoryItem', data.item.id, data.player, data.item.label)
            Wait(250)
            loadPlayerInventory(currentMenu)
            ESX.ShowNotification('Vous venez de donner votre ~r~'..data.item.label)
        else
            if data.item.label == "Argent" then
                data.item.name = "cash"
            end
            ...
            if TriggerServerEvent("esx:giveInventoryItem", data.player, data.item.type, data.item.name, count) then
                RequestAnimDict('mp_common')
                TaskPlayAnim(ped, 'mp_common', 'givetake1_b', 8.0, -8, 10.0, 49, 0, 0, 0, 0)
                Citizen.Wait(1200)
                ClearPedSecondaryTask(ped)
                Wait(250)
                loadPlayerInventory(currentMenu)
...
RegisterNUICallback("weightHelp", function(data, cb)
    ESX.ShowNotification("Pour avoir plus ~q~d'espace~s~, vous pouvez toujours obtenir un ~q~diable")
end)
...
local tenue,chaussures,masque,pantalon,tshirt,lunettes,gants,modifitems,chapeau,sac,chaine,calque,montre,torse,oreille,gilet = {},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}
...
function loadPlayerInventory(result)
    ...
    if result == 'item' then 
        ESX.TriggerServerCallback("esx_inventoryhud:getPlayerInventory", function(data)
            ...
            if dirtycash ~= nil and dirtycash > 0 then
                dirtycashData = {
                    label = "Argent de source inconnue",
                    name = "dirtycash",
                    type = "item_account",
                    count = dirtycash,
                    usable = false,
    ...
    elseif result == 'clothe' then 
        ...
        ESX.TriggerServerCallback('SneakyLife:GetOutfit', function(Vetement)
            tenue, chaussures, masque, pantalon, tshirt, lunettes, gants, chapeau, sac,chaine,calque,montre, torse, oreille, gilet = {}, {}, {}, {}, {}, {}, {}, {}, {}, {},{},{}, {}, {}, {}
...
--[[                if v.type == "peelochaussures" then 
                ...
                if v.type == "peelopantalon" then 
                    table.insert(pantalon, {name = v.nom, skins = json.decode(v.clothe), id = v.id, data = "pants_1", data2 = "pants_2", decals = 21})
                end
                if v.type == "peelotshirt" then 
                    table.insert(tshirt, {name = v.nom, skins = json.decode(v.clothe), id = v.id, data = "tshirt_1", data2 = "tshirt_2", decals = 15})
                end 
                if v.type == "peelogant" then 
                    table.insert(gants, {name = v.nom, skins = json.decode(v.clothe), id = v.id, data = "arms", data2 = "arms_2", decals = 15})
                end --]]
                ...
                if v.type == "peeloomontre" then 
                    table.insert(montre, {name = v.nom, skins = json.decode(v.clothe), id = v.id, data = "watches_1", data2 = "watches_2", decals = 2})
                end 
                if v.type == "peeloogilet" then 
                    table.insert(gilet, {name = v.nom, skins = json.decode(v.clothe), id = v.id, data = "bproof_1", data2 = "bproof_2"})
                end 
                if v.type == "peelooreille" then 
...
function StartAnimsWeapons2(weapons)
    local playerPed = GetPlayerPed(-1)
    SetPedCurrentWeaponVisible(playerPed, false)
    bool1 = true
    StartDisableControlForWeaponAnim(true)
    TaskAnimForce({"reaction@intimidation@1h", "outro"}, 49)
...
function StartsPoliceAnimsWeapons2(weapons)
    loadAnimDict("rcmjosh4")
    loadAnimDict("reaction@intimidation@cop@unarmed")
    local playerPed = GetPlayerPed(-1)
    local hash = GetHashKey(weapons)
    ...
    if fastWeapons[1] ~= nil then
        ...
        if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(fastWeapons[1]) then
            ...
            if fastWeapons[1] == "WEAPON_STUNGUN" or fastWeapons[1] == "WEAPON_COMBATPISTOL" then
                StartsPoliceAnimsWeapons2(fastWeapons[1])
            else
                StartAnimsWeapons2(fastWeapons[1],true)
            end
            notifWeapon1 = ShowAboveRadarMessage('Vous avez rangé votre ~g~'..ESX.GetWeaponLabel(fastWeapons[1])..'.')
        else
            if fastWeapons[1] == "WEAPON_STUNGUN" or fastWeapons[1] == "WEAPON_COMBATPISTOL" then
    ...
    if fastWeapons[2] ~= nil then
        if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(fastWeapons[2]) then
            ...
            if fastWeapons[2] == "WEAPON_STUNGUN" or fastWeapons[2] == "WEAPON_COMBATPISTOL" then
                StartsPoliceAnimsWeapons2(fastWeapons[2])
            else
                StartAnimsWeapons2(fastWeapons[2],true)
            end
            SetCurrentPedWeapon(GetPlayerPed(-1), "WEAPON_UNARMED",true)
            notifWeapon1 = ShowAboveRadarMessage('Vous avez rangé votre ~g~'..ESX.GetWeaponLabel(fastWeapons[2])..'.')
        else
            if fastWeapons[2] == "WEAPON_STUNGUN" or fastWeapons[2] == "WEAPON_COMBATPISTOL" then
    ...
    if fastWeapons[3] ~= nil then
        if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(fastWeapons[3]) then
            ...
            if fastWeapons[3] == "WEAPON_STUNGUN" or fastWeapons[3] == "WEAPON_COMBATPISTOL" then
                StartsPoliceAnimsWeapons2(fastWeapons[3])
            else
                StartAnimsWeapons2(fastWeapons[3],true)
            end
            SetCurrentPedWeapon(GetPlayerPed(-1), "WEAPON_UNARMED",true)
            notifWeapon1 = ShowAboveRadarMessage('Vous avez rangé votre ~g~'..ESX.GetWeaponLabel(fastWeapons[3])..'.')
        else
            if fastWeapons[3] == "WEAPON_STUNGUN" or fastWeapons[3] == "WEAPON_COMBATPISTOL" then
