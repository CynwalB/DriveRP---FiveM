...
AddEventHandler("esx_inventoryhud:openTrunkInventory", function(data, blackMoney, money, inventory, weapons)
    setTrunkInventoryData(data, blackMoney, money, inventory, weapons)
    openTrunkInventory(trunkType)
end)

RegisterNetEvent("esx_inventoryhud:refreshTrunkInventory")
AddEventHandler("esx_inventoryhud:refreshTrunkInventory",function(data, blackMoney, money, inventory, weapons)
    setTrunkInventoryData(data, blackMoney, money, inventory, weapons)
end)
...
function openmenuvehicle()
    ...
    if locked == 1 then
        ...
        ESX.ShowNotification("Vous êtes trop ~r~loin~s~ ou ce coffre est ~r~fermé~s~.")
    end
end
...
function setTrunkInventoryData(data, blackMoney, money, inventory, weapons)
    trunkData = data
    ...
    if trunkType == 'item' then
        ...
        if blackMoney and blackMoney > 0 then
            accountData = {
