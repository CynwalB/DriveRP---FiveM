...
function getTotalInventoryWeight(plate)
    ...
    TriggerEvent("esx_inventoryhud_trunk:getSharedDataStore",plate,function(store)
        ...
        local W_money = 0
        local Account = (store.get("cash")) or 0
        if Account ~= 0 then
            W_money = Account[1].amount / 10
        end
...
ESX.RegisterServerCallback("esx_inventoryhud_trunk:getInventoryV",function(source, cb, plate)
    ...
    cb({blackMoney = blackMoney, money = money,items = items,weapons = weapons,weight = weight})
    end)
end)
                ...
                if coffre[i].name == item then
                    if (coffre[i].count >= count and count > 0) then
                        ...
                        if coffre[i].canMove == nil or coffre[i].canMove then
                            ...
                            exports.sCore:SendLogs(1752220,"Coffre véhicule",""..GetPlayerName(_source).." vient de retirer "..count.." de **"..item.."** dans le coffre **"..plate.."** \n License : "..xPlayer.identifier,"https://discord.com/api/webhooks/1151182751144353855/l9AMvIAWRN-VKwnqciyUTqSuorNvsIt47ct3r3GnEQpXlbFbBps1Ttv3InC05339u1vM")
        ...
        TriggerEvent("esx_inventoryhud_trunk:getSharedDataStore",plate,function(store)
            if item == "dirtycash" then
                ...
                if (blackMoney[1].amount >= count and count > 0) then
                    ...
                    exports.sCore:SendLogs(1752220,"Coffre véhicule",""..GetPlayerName(_source).." vient de retirer "..count.." de **l'argent sale** dans le coffre **"..plate.."** \n License : "..xPlayer.identifier,"https://discord.com/api/webhooks/1151182751144353855/l9AMvIAWRN-VKwnqciyUTqSuorNvsIt47ct3r3GnEQpXlbFbBps1Ttv3InC05339u1vM")
                    local blackMoney = 0
                    local items = {}
                    local weapons = {}
                    weapons = (store.get("weapons") or {})
            ...
            elseif item == "money" then
                item = "cash"
                local money = store.get("cash")
                if (money[1].amount >= count and count > 0) then
                    money[1].amount = money[1].amount - count
                    store.set("cash", money)
                    xPlayer.addAccountMoney("cash", count)
                    exports.sCore:SendLogs(1752220,"Coffre véhicule",""..GetPlayerName(_source).." vient de retirer "..count.." de **l'espèce** dans le coffre **"..plate.."** \n License : "..xPlayer.identifier,"https://discord.com/api/webhooks/1151182751144353855/l9AMvIAWRN-VKwnqciyUTqSuorNvsIt47ct3r3GnEQpXlbFbBps1Ttv3InC05339u1vM")
                    local money = 0
                    local items = {}
                    local weapons = {}
                    weapons = (store.get("weapons") or {})
...
--            print("Argent")
            ...
            if (playerAccountMoney >= count and count > 0) then
                TriggerEvent("esx_inventoryhud_trunk:getSharedDataStore",plate,function(store)
                    local money = (store.get("cash") or nil)
                    if money ~= nil then
                        money[1].amount = money[1].amount + count
                    else
                        money = {}
                        table.insert(money, {amount = count})
                    end
                        xPlayer.removeAccountMoney(item, count)
                        store.set("cash", money)
                        exports.sCore:SendLogs(1752220,"Coffre véhicule",""..GetPlayerName(_source).." vient de déposer "..count.." de **l'espèce** dans le coffre **"..plate.."** \n License : "..xPlayer.identifier,"https://discord.com/api/webhooks/1151182751144353855/l9AMvIAWRN-VKwnqciyUTqSuorNvsIt47ct3r3GnEQpXlbFbBps1Ttv3InC05339u1vM")
                        MySQL.Async.execute(
                            "UPDATE truck_inventory2 SET owned = @owned WHERE plate = @plate",
                            {
                                ["@plate"] = plate,
...
--            print("Argent sale")
            local playerAccountBlackMoney = xPlayer.getAccount(item).money
            if (playerAccountBlackMoney >= count and count > 0) then
                TriggerEvent("esx_inventoryhud_trunk:getSharedDataStore",plate,function(store)
                    ...
                    if blackMoney ~= nil then
                        ...
                        xPlayer.removeAccountMoney(item, count)
                        store.set("dirtycash", blackMoney)
                        exports.sCore:SendLogs(1752220,"Coffre véhicule",""..GetPlayerName(_source).." vient de déposer "..count.." de **l'argent sale** dans le coffre **"..plate.."** \n License : "..xPlayer.identifier,"https://discord.com/api/webhooks/1151182751144353855/l9AMvIAWRN-VKwnqciyUTqSuorNvsIt47ct3r3GnEQpXlbFbBps1Ttv3InC05339u1vM")
                        MySQL.Async.execute(
                            "UPDATE truck_inventory2 SET owned = @owned WHERE plate = @plate",
                            {
                                ["@plate"] = plate,
...
ESX.RegisterServerCallback("esx_inventoryhud_trunk:getPlayerInventory",function(source, cb)
    ...
    cb({blackMoney = blackMoney, money = money, items = items})
end)