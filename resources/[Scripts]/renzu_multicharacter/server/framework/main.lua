...
GetCharacters = function(source,data,slots)
	...
	if Config.framework == 'ESX' then
		...
		local license = xPlayer.getIdentifier(source)
...
DeleteCharacter = function(source,slot)
	if Config.framework == 'ESX' then
		xPlayer = ESX.GetPlayerFromId(source)
		local identifier = Config.Prefix..'%:'..xPlayer.getIdentifier(source)
...
Command = function(command)
	if Config.framework == 'ESX' then
		RegisterCommand('admin', function(xPlayer, args, showError)
