...
AddEventHandler("rx:BuyLicense", function(price)
    ...
    if xPlayer.getAccount('cash').money >= price then
        ...
        TriggerClientEvent('esx:showNotification', source, ("[~r~Erreur~s~] Vous n'avez pas assez d'argent il vous manque ~r~%s$~s~."):format(price - xPlayer.getAccount('cash').money))
    end
end)
