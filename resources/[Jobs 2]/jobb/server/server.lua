...
Citizen.CreateThread(function()
	...
	AddEventHandler('rxeJobBuilder:recolte', function(item)
		local _source = source
		local xPlayer = ESX.GetPlayerFromId(_source)
		local result = xPlayer.getInventoryItem(item).weight
    	local weight = xPlayer.getWeight()
		local a = result + weight
		if a >= xPlayer.getMaxWeight() then
	...
	AddEventHandler('rxeJobBuilder:processing', function(itemProcessing, itemReward)
		...
		if xItem > 0 then
			xPlayer.removeInventoryItem(itemProcessing, 3)
			xPlayer.addInventoryItem(itemReward, 3)
		else
